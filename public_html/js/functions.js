...
function __read_number(input_element, use_page_currency = false) {
    if(input_element.is('input')){
        ...
        return __number_uf(input_element.html(), use_page_currency);
    }
}

function round_init(number, digit) {
    return Math.round(parseInt(number)/Math.pow(10, digit)) * Math.pow(10, digit);
}
...
function __write_number(
    ...
    if(input_element.is('input')){
        ...
        input_element.html(__number_f(value, false, use_page_currency, precision));
    }
}
...
//---- CUSTOM JS ----//
//Convert size (width, height, area)
/*function __size_uf(input, use_page_currency = false) {
    if (use_page_currency && __currency_decimal_separator) {
        var decimal = __p_currency_decimal_separator;
    } else {
        var decimal = __currency_decimal_separator;
...
function __size_f(
    ...
    show_symbol = false,
    use_page_currency = false,
    precision = __size_precision
) {
    return __currency_trans_from_en(input, show_symbol, use_page_currency, precision);
}

/*function __read_size(input_element, use_page_currency = false) {
    if(input_element.is('input')){
        return __size_uf(input_element.val(), use_page_currency);
    }else{
        return __size_uf(input_element.html(), use_page_currency);
    }
}*/

function __write_size(
    input_element,
    value,
    use_page_currency = false,
    precision = __size_precision
    ...
    if(input_element.is('input')){
        input_element.val(__size_f(value, false, use_page_currency, precision));
    }else{
        input_element.html(__size_f(value, false, use_page_currency, precision));
    }
}

//Convert weight
/*function __weight_uf(input, use_page_currency = false) {
    if (use_page_currency && __currency_decimal_separator) {
        var decimal = __p_currency_decimal_separator;
    } else {
        var decimal = __currency_decimal_separator;
...
function __weight_f(
    ...
    show_symbol = false,
    use_page_currency = false,
    precision = __weight_precision
) {
    return __currency_trans_from_en(input, show_symbol, use_page_currency, precision);
}

/*function __read_weight(input_element, use_page_currency = false) {
    if(input_element.is('input')){
        return __weight_uf(input_element.val(), use_page_currency);
    }else{
        return __weight_uf(input_element.html(), use_page_currency);
    }
}*/

function __write_weight(
    input_element,
    value,
    use_page_currency = false,
    precision = __weight_precision
    ...
    if(input_element.is('input')){
        input_element.val(__weight_f(value, false, use_page_currency, precision));
    }else{
        input_element.html(__weight_f(value, false, use_page_currency, precision));
    }
}
