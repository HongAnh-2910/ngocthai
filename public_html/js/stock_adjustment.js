$(document).ready(function() {
    function calculate_total(tr){
        ...
        let quantity = parseInt(__number_uf(tr.find('.adjustment_quantity').val()));
        let total_quantity = 0;
        let total_weight = 0;

        if (unit_type == 'pcs'){
            total_quantity += quantity;
        }else if (unit_type == 'meter' || unit_type == 'area'){
            let weight = parseFloat(tr.find('.weight').val());

            let width_before = parseFloat(tr.find('.origin_width').val());
            let width_after = parseFloat(tr.find('.adjustment_width').val());

            let height_before = parseFloat(tr.find('.origin_height').val());
            let height_after = parseFloat(tr.find('.adjustment_height').val());

            total_weight += (width_before * height_before * quantity - width_after * height_after * quantity) * weight;
        }

        __write_number(tr.find('.weight_adjustment'), total_weight, false, 2);
        __write_number(tr.find('.pcs_adjustment'), total_quantity);

        let max_weight_dont_need_confirm = parseFloat($('#max_weight_dont_need_confirm').val());
        let max_pcs_dont_need_confirm = parseInt($('#max_pcs_dont_need_confirm').val());

        if(total_weight > max_weight_dont_need_confirm){
            tr.find('.weight_adjustment_box').attr('style', 'color:#f00;');
        }else{
            tr.find('.weight_adjustment_box').removeAttr('style');
        }

        if(total_quantity > max_pcs_dont_need_confirm){
            tr.find('.pcs_adjustment_box').attr('style', 'color:#f00;');
        }else{
            tr.find('.pcs_adjustment_box').removeAttr('style');
        }
    }

    function calculate_total_for_all(){
    ...
    $('#stock_adjustment_product_table').on('change', '.adjustment_height, .adjustment_quantity', function(){
        let tr = $(this).closest('tr');
        calculate_total(tr);
    });

    $('#stock_adjustment_product_table').on('change', '.adjustment_width', function(){
        let tr = $(this).closest('tr');
        let current_width = parseFloat(tr.find('.current_width').html());
        let adjustment_width = parseFloat($(this).val());

        if(adjustment_width > current_width){
            // tr.attr('style', 'color:#f00;');
            // toastr.warning(LANG.adjustment_width_not_valid);
            tr.find('.remaining_width').html(0);
        }else{
            // tr.removeAttr('style');
            tr.find('.remaining_width').html(current_width - adjustment_width);
        }
        calculate_total(tr);
    });
    ...
    plate_stock_deliver_table = $('#plate_stock_deliver_table').DataTable({
        ...
        ajax: {
            ...
            data: function(d) {
                d.location_id = $('#adjust_location_id').val();
                d.warehouse_id = $('#adjust_warehouse_id').val();
        ...
        //     $('#footer_total_stock').html(__sum_stock($('#plate_stock_deliver_table'), 'current_stock'));
        //     var total_stock_price = sum_table_col($('#plate_stock_deliver_table'), 'total_stock_price');
        //     $('#footer_total_stock_price').text(total_stock_price);
        //
        //     __currency_convert_recursively($('#plate_stock_deliver_table'));
        // },
    });

    $('#adjust_warehouse_id, #adjust_location_id, #plate_stock_deliver_filter_form #category_id, #plate_stock_deliver_filter_form #view_stock_filter, #plate_stock_deliver_filter_form #plate_width, #plate_stock_deliver_filter_form #plate_height, #plate_stock_deliver_filter_form #variation_id, #plate_stock_deliver_filter_form #quantity'
    ).change(function() {
    ...
    $('#plate_stock_deliver_table tbody').on('click', '.view_detail', function() {
        ...
        let location_id = $('#adjust_location_id').val();
        let warehouse_id = $('#adjust_warehouse_id').val();
    ...
    function changeLocation(location_id){
        if($('select#adjust_location_id').val() != undefined){
            if ($('select#adjust_location_id').val()) {
    ...
    $('select#adjust_location_id').change(function() {
        changeLocation();
    });
    ...
    $('#plate_stock_deliver_modal').on('click', '#select_plates', function() {
        ...
        $.ajax({
            method: 'POST',
            url: '/stock-adjustments/get_sell_entry_row',
            dataType: 'html',
            data: {
            ...
            success: function(result) {
                if(result != ''){
                    ...
                    if(error == ''){
                        ...
                        getFormatNumber();
                        let new_tr = $('#stock_adjustment_product_table tbody tr').last();
                        calculate_total(new_tr);
                        // let new_tr = $('#stock_adjustment_product_table tbody tr').last();
                        calculate_total_for_all();
                    }else{
                        swal({
                            title: error,
                            icon: 'warning',
    ...
    $('#save_stock_adjustment').click(function(e) {
        e.preventDefault();
        ...
        $('#stock_adjustment_product_table .product_row').each(function(){
            let tr = $(this);
            let current_width = parseFloat(tr.find('.current_width').html());
            let adjustment_width = parseFloat(tr.find('.adjustment_width').val());
        ...
        if ($('form#stock_adjustment_form').valid() && !error) {
            $('#save_stock_adjustment').attr('disabled', true);
    ...
    $(document).on('click', '.approve_stock_adjustment', function(e) {
        let tr = $(this).closest('tr');
        let url = $(this).data('href');

        swal({
            title: LANG.confirm_stock_adjustment_message,
            icon: 'warning',
            buttons: true,
            dangerMode: true,
        }).then(willDelete => {
...
function stock_adjustment_product_row(variation_id) {
    ...
    var location_id = $('select#adjust_location_id').val();
    $.ajax({
        method: 'POST',
