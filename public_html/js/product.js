...
function changeUnit(unit_id) {
    $.ajax({
        method: 'get',
        url: '/products/get-sub-units-by-unit',
        dataType: 'json',
        data: {
            unit_id: unit_id
        },
        success: function (result) {
            ...
            $('#default_sub_unit_id').empty();
            $('#default_sub_unit_id').append('<option value="">' + LANG.select + '</option>');

            for (let i = 0; i < key.length; i++) {
                let selected = '';
                if(result[i].id == unit_id){
                    selected = 'selected';
                }
                $('#default_sub_unit_id').append('<option value="' + result[i].id + '" '+ selected +'>' + result[i].name + '</option>');
            }
        }
    });
}
...
$(document).ready(function() {
    $('#unit_id').change(function(){
        ...
        var unit_type = $(this).find(':selected').data('unit_type');

        if(unit_type == 'area' || unit_type == 'meter'){
            changeUnit(unit_id);
            $('#default_sub_unit_id_box').show();
            $('#thickness_box').show();
            $('#weight_box').show();
        }else{
            $('#default_sub_unit_id_box').hide();
            $('#thickness_box').hide();
            $('#weight_box').hide();
        }

        if (unit_type == 'area') {
            $('.price_by_plate').css('display', 'block');
            $('.price_by_plate_label').text('Giá bán theo mét vuông');
        }if (unit_type == 'meter') {
            $('.price_by_plate').css('display', 'block');
            $('.price_by_plate_label').text('Giá bán theo mét');
        } else {
            $('.price_by_plate').css('display', 'none');
            $('.price_by_plate_label').text('Giá bán');
        }

        if(unit_type == 'service'){
            $('#enable_stock').iCheck('uncheck');
            $('#enable_stock_box').hide();
            $('#alert_quantity_div').hide();
        }else{
            $('#enable_stock').iCheck('check');
            $('#enable_stock_box').show();
    ...
    $('#sub_unit_ids').select2({
        ...
        templateSelection : function (tag, container){
            var $option = $('#sub_unit_ids.select2 option[value="'+tag.id+'"]');
    ...
    $('#upload_file_sales').fileinput(sales_fileinput);
