...
$(document).ready(function() {
    ...
    if ($('#status_span').length) {
        ...
        if (status === '1') {
            var receipt_json = $('#status_span').attr('data-receipt');
            if(receipt_json != ''){
                var receipt = JSON.parse(receipt_json);
                initialize_printer();
                pos_print(receipt);
    ...
    if ($('input#__size_precision').length > 0) {
        __size_precision = $('input#__size_precision').val();
    } else {
        __size_precision = 2;
    }

    if ($('input#__weight_precision').length > 0) {
        __weight_precision = $('input#__weight_precision').val();
    } else {
        __weight_precision = 2;
    }
...
ranges[LANG.tomorrow] = [moment().subtract(-1, 'days'), moment().subtract(-1, 'days')];
...
var dateRangeRevenueSettings = {
    ranges: ranges,
    startDate: receipt_today.start,
    endDate: receipt_today.end,
    locale: {
        cancelLabel: LANG.clear,
        applyLabel: LANG.apply,
        customRangeLabel: LANG.custom_range,
