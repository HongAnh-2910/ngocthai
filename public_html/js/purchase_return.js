...
function calculateArea(tr){
    ...
    var quantity = tr.find('.purchase_quantity').val();
    var weight = tr.find('.weight').val();
    var purchase_unit_cost = $('.purchase_unit_cost_hidden').val();
    ...
    $.ajax({
        ...
        success: function(result) {
            if (result.success == true) {
                tr.find('.area_hidden').val(result.data);
                tr.find('.area').text(result.data);
                tr.find('.weight').text(result.weight);
                updateTotal(tr);
            }
        },
    });
...
$('table#purchase_entry_table').on('change', '.width', function() {
    var tr = $(this).closest('tr');
    calculateArea(tr);
});

$('table#purchase_entry_table').on('change', '.height', function() {
    var tr = $(this).closest('tr');
    calculateArea(tr);
});

$('table#purchase_entry_table').on('change', '.purchase_quantity', function() {
    var tr = $(this).closest('tr');
    calculateArea(tr);
});
