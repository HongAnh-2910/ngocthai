//Common
// class Common {
//     ajax = function(url, params, successFunc, doneFunc, errorFunc){
//         $.ajax({
//             url : url,
//             type : 'POST',
//             dataType : 'json',
...
$(document).ready(function() {
    ...
    $(document).on( 'click', '.payment_closed', function(e){
        toastr.error(LANG.not_update_payment_after_closed);
    });

    $(document).on('click', '#close_end_of_day_button', function(){
        swal({
            title: LANG.sure,
            text: LANG.close_end_of_day_message,
            icon: 'warning',
            buttons: true,
            dangerMode: true,
        }).then(willDelete => {
            if (willDelete) {
                $('#close_end_of_day_form').submit();
            }
        });
    });

    $(document).on('click', '#payment-button, #reject-button', function() {
    ...
    //Change user role
    $('#user_add_form #role, #user_edit_form #role').change(function(){
        let user_form = $(this).closest('form');
    ...
    $('.unit_modal').on('shown.bs.modal', function(e) {
        $('form#unit_add_form').submit(function(e) {
            e.preventDefault();
        })
            .validate({
                rules: {
                    actual_name: {
                        required: true,
                        remote: {
                            url: '/units/check-unit-existed',
                            type: 'post',
                            data: {
                                actual_name: function() {
                                    return $('#actual_name').val();
                                },
                                type_validate: function() {
                                    return $('#type_validate').val();
                                },
                                unit_id: function() {
                                    return $('#unit_id').val();
                                }
                ...
                message: {
                    actual_name: {
                        remote: LANG.unit_already_exists
                    }
                },
                submitHandler: function(form) {
                    var define_base_unit = $('form#unit_add_form #define_base_unit').is(':checked');
                    var type = $('#type').val();
                    var base_unit_id = $('#base_unit_id').val();

                    if(define_base_unit){
                        if(type == 'area'){
                            var height = __read_number($('#height'));
                            var width = __read_number($('#width'));

                            if(height <= 0){
                                toastr.error(LANG.height_require);
                                return;
                            }else if(width <= 0){
                                toastr.error(LANG.width_require);
                                return;
                            }
                        }else{
                            if(base_unit_id == ''){
                                toastr.error(LANG.base_unit_require);
                                return;
                            }
                        }
                    }
        ...
        $('div.unit_modal').load($(this).data('href'), function() {
            ...
            $('form#unit_edit_form').submit(function(e) {
                ...
                .validate({
                    ...
                    submitHandler: function(form) {
                        var define_base_unit = $('#define_base_unit').is(':checked');
                        var type = $('#type').val();
                        var base_unit_id = $('#base_unit_id').val();
    ...
    if (contact_table_type == 'supplier') {
        ...
        Array.prototype.push.apply(columns, [
            ...
            { data: 'price_group', name: 'price_group', searchable: false },
        ...
        buttons: [
            {
                ...
                customize: function( xlsx, row ) {
                    ...
                    if (contact_table_type === 'customer'){
                        $('row c[r^="G"], row c[r^="M"]', sheet).attr( 's', 63);
                    }else{
                        $('row c[r^="G"], row c[r^="J"]', sheet).attr( 's', 63);
                    }
                },
            },
            {
    ...
    $('.contact_modal').on('shown.bs.modal', function(e) {
        ...
        } else if ($('select#contact_type').val() == 'both') {
            $('div.supplier_fields').fadeIn();
            $('div.customer_fields').fadeIn();
            ...
            .validate({
                rules: {
                    ...
                    selling_price_group_id: {
                        remote: {
                            url: '/contacts/check-price-group',
                            type: 'post',
                            data: {
                                selling_price_group_id: function() {
                                    return $('#selling_price_group_id').val();
                                },
                                type: function() {
                ...
                messages: {
                    ...
                    mobile: {
                        remote: LANG.mobile_already_exists,
                    },
                    tax_number: {
                        remote: LANG.tax_number_already_exists,
                    },
                    selling_price_group_id: {
                        remote: LANG.selling_price_group_is_null,
                    }
                },
                submitHandler: function(form) {
                    e.preventDefault();
    ...
    //Add reduce debt
    $(document).on('submit', 'form#reduce_debt_form', function(e) {
        e.preventDefault();
        $(this)
            .find('button[type="submit"]')
            .attr('disabled', true);
    ...
    function getAccounts(payment_type){
        console.log('ok');
        $('.account_id').prop('disabled', true);

        $.ajax({
            method: 'POST',
            url: '/sells-of-cashier/get-accounts',
            dataType: 'html',
            data: { method: payment_type },
            success: function(result) {
                if (result) {
    ...
    $(document).on('change', '.payment_types_dropdown', function() {
        ...
        getAccounts(payment_type);

        if(payment_type == 'bank_transfer'){
            $.ajax({
                method: 'GET',
                url: '/payments/check-confirm-bank-transfer-permission',
                dataType: 'json',
                success: function(result) {
                    if (result.success == true){
                        $('#reject-button').show();
    ...
    $(document).on('change', '.receipt_payment_types_dropdown, .expense_payment_types_dropdown', function() {
    ...
    $(document).on('click', 'button.delete_commsn_agnt_button', function() {
        swal({
            title: LANG.sure,
            text: LANG.confirm_delete_commsn_agnt,
            icon: 'warning',
            buttons: true,
            dangerMode: true,
        }).then(willDelete => {
    ...
    $(document).on('click', '.delete-sale', function(e) {
        ...
        swal({
            title: LANG.sure,
            text: LANG.confirm_delete_sell_pos,
            icon: 'warning',
            buttons: true,
            dangerMode: true,
        }).then(willDelete => {
    ...
    // Approved Cashier
    $(document).on('click', '.reject_sell_return', function(e) {
        e.preventDefault();
        ...
        swal({
            ...
            dangerMode: true,
            title: LANG.sure,
            text: LANG.confirm_reject_return,
            icon: "warning",
        }).then(result => {
            if (result) {
                $.ajax({
                    type: 'POST',
                    url: '/sell-return/reject/' + id,
                    dataType: 'json',
                    data: {
                        id: id
                    },
    ...
    $(document).on('click', '#confirm-export-button', function(e) {
        let stock_deliver_form = $(this).closest('form');
        e.preventDefault();
        swal({
            buttons: true,
            dangerMode: true,
            title: LANG.sure,
            text: LANG.confirm_export_stock,
            icon: "warning",
        }).then(result => {
            if (result) {
                stock_deliver_form.submit()
            }
        });
    });

    $(document).on('click', '#approval-return-button', function(e) {
        let sell_return_form = $(this).closest('form');
        e.preventDefault();
        swal({
            buttons: true,
            dangerMode: true,
            title: LANG.sure,
            text: LANG.confirm_approved_return,
            icon: "warning",
        }).then(result => {
            if (result) {
    ...
    //Cancel sell order
    $(document).on('click', '.cancel_sell', function(e) {
        e.preventDefault();
        let link = $(this).data('href');
        let id = link.split('/').pop();

        /*let has_payment = parseInt($(this).data('has_payment'));
        let message;
        if(has_payment == 1){
            message = LANG.confirm_cancel_sell_pos_with_payment;
        }else{
            message = LANG.confirm_cancel_sell_pos;
        }*/

        $.ajax({
            type: 'get',
            url: '/sells/check-invoice-cancelled',
            dataType: 'json',
            data: {
                id: id
            },
            success: function(result) {
                if (result.success) {
                   /* if (result.data.is_deliver == 1) {
                        toastr.error(LANG.invoice_delivered);
                    } else {*/
    ...
    // Approved Cashier
    $(document).on('click', '.approve-cashier', function(e) {
        ...
        swal({
            ...
            dangerMode: true,
            title: LANG.sure,
            text: LANG.confirm_approved_cashier,
            icon: "warning",
        }).then(result => {
            if (result) {
                $.ajax({
                    type: 'POST',
                    url: '/sells/pos/approve_cashier/' + id,
                    dataType: 'json',
                    data: {
                        id: id
                    },
...
$(document).on('click', 'a.delete_purchase_return', function(e) {
    ...
    swal({
        title: LANG.sure,
        text: LANG.confirm_delete_purchase_return,
        icon: 'warning',
        buttons: true,
        dangerMode: true,
    }).then(willDelete => {
...
function updateRevenue(start = null, end = null, location_id = null, selector = null) {
    if(start == null){
        var start = $('#revenue_date_filter')
            .data('daterangepicker')
            .startDate.format('YYYY-MM-DD');
    }
    if(end == null){
        var end = $('#revenue_date_filter')
            .data('daterangepicker')
            .endDate.format('YYYY-MM-DD');
    }
    if(location_id == null){
        var location_id = $('#revenue_location_filter').val();
    }
    var data = { start_date: start, end_date: end, location_id: location_id };
    selector = selector == null ? $('#revenue_data_div') : selector;
    var loader = '<div class="text-center">' + __fa_awesome() + '</div>';
    selector.html(loader);
    $.ajax({
        method: 'GET',
...
function getFormatNumber() {
    $('input.input_number').keyup(function(event) {
        // skip for arrow keys
    ...
    getFormatDecimalNumber();
}

function getFormatDecimalNumber(){
    $('input.input_decimal').on('keypress', function(event) {
        var keyCode = event.keyCode || event.which;

        if (keyCode === 45 || keyCode === 189 || keyCode === 101) {
...
$(document).on('click', '.update_cod_button', function(e) {
    e.preventDefault();
    var error = false;
    if ($('#cod_amount').val() < 0) {
        toastr.error(LANG.amount_must_greater_than_0);
        error = true;
    }

    if (!error) {
        $('#update_cod_form').submit();
    }
});

$(document).on('click', '.store_debit_paper_button', function(e) {
    $('#store_debit_paper_form').submit();
});

function validateReverseQuantity(){
        ...
        success: function(result) {
            if(result.success){
                if(reverse_quantity > 0 && reverse_quantity <= result.data){
                    // is_valid = true;
                    $('#reverse_quantity').css('border-color', '#d2d6de');
                    $('#reverse_confirm').removeAttr('disabled');
                }else{
                    $('#reverse_quantity').css('border-color', 'red');
                    $('#reverse_confirm').attr('disabled', true);
