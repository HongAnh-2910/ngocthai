<div class="modal-dialog modal-lg" role="document">
  ...
  @php
    ...
    if (request()->segments()[0] == 'contacts') {
        $url_index = request()->query();
        if ($url_index['type'] == 'customer') {
            $type_contact = 'customer';
        } elseif ($url_index['type'] == 'supplier') {
            $type_contact = 'supplier';
        }
    }

    $supplier = isset($type_contact) && $type_contact == 'supplier';

  @endphp
    {!! Form::open(['url' => $url, 'method' => 'post', 'id' => $form_id ]) !!}
      <input type="hidden" name="type_validate" id="type_validate">
    <div class="modal-header">
      <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
      <h4 class="modal-title">{{ $supplier ? __('contact.add_supplier') : __('contact.add_customer') }}</h4>
    </div>
    ...
    <div class="modal-body">
      ...
      {{--{!! Form::hidden('type', $supplier ? \App\Contact::SUPPLIER : \App\Contact::CUSTOMER , ['class' => 'form-control', 'id' => 'contact_type','placeholder' => __('messages.please_select'), 'required']); !!}--}}
      <div class="col-md-6 contact_type_div">
        <div class="form-group">
      ...
      <div class="col-md-4 supplier_fields" style="{{ $supplier ? 'display: block' : 'display: none' }}">
          <div class="form-group">
      ...
      <div class="col-md-4 customer_fields" style="{{ $supplier ? 'display: none' : 'display: block' }}">
          <div class="form-group">
              {!! Form::label('selling_price_group_id', __('lang_v1.selling_price_group') . '*:') !!}
              {!! Form::select('selling_price_group_id', $selling_price_groups, '', ['class' => 'form-control', 'required']); !!}
          </div>
      </div>
      <div class="col-md-4">
        <div class="form-group">
      ...
      <div class="clearfix"></div>
          @if(isset($quick_add) || (request()->segments()[0] == 'contacts' && (request()->query())['type'] == 'customer'))
          <div class="col-md-12"><hr></div>
          <div class="col-md-12" >
              {!! Form::label('landmark', __('lang_v1.shipping_address') . ':') !!}
