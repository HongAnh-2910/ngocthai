						...
						@if(count($warehouses) > 0)
							@foreach($warehouses as $warehouse)
								@php
									...
									if (count($warehouse->plates) > 1) {
										$rowspan = 'rowspan='. count($warehouse->plates);
									} else {
										$rowspan = '';
									}

									if (!empty($warehouse->plates)) {
										foreach ($warehouse->plates as $key => $plate) {
											if ($key == 0) {
												$plate_line_first =  $plate;
											} else {
												$plate_line_remaining[] = $plate;
											}
										}
									}
								@endphp
								<tr>
									...
									@if($plate_line_first)
										<td>
											{{ $plate_line_first['product'] }}
										</td>
										<td>
											@if($plate_line_first['unit_type'] == 'pcs')
												{{ $plate_line_first['total_area_remaining'] }} {{ $plate_line_first['unit_name'] }}
											@else
												{{ @number_format($plate_line_first['total_area_remaining'], 2) }} m<sup>2</sup>
											@endif
										</td>
									@endif
								</tr>
								...
								@if(!empty($plate_line_remaining))
									@foreach($plate_line_remaining as $plate_stock)
										<tr>
											<td>
												{{ $plate_stock['product'] }}
											</td>
											<td>
												@if($plate_stock['unit_type'] == 'pcs')
													{{ $plate_stock['total_area_remaining'] }} {{ $plate_stock['unit_name'] }}
												@else
													{{ @number_format($plate_stock['total_area_remaining'], 2) }} m<sup>2</sup>
												@endif
											</td>
										</tr>
									@endforeach
