...
@endif
@if(empty($only) || in_array('export_statuses', $only))
    <div class="col-md-3">
        <div class="form-group">
            {!! Form::label('export_status', __('lang_v1.export_status') . ':') !!}
            {!! Form::select('export_status', $export_statuses, null, ['class' => 'form-control select2', 'style' => 'width:100%', 'placeholder' => __('lang_v1.all')]); !!}
        </div>
    </div>
@endif
@if(empty($only) || in_array('sell_list_filter_variation_id', $only))
    <div class="col-md-3">
        <div class="form-group">
            {!! Form::label('sell_list_filter_category_id', __('category.category') . ':') !!}
            {!! Form::select('sell_list_filter_category', $categories, null, ['placeholder' => __('messages.all'), 'class' => 'form-control', 'style' => 'width:100%', 'id' => 'sell_list_filter_category_id']); !!}
        </div>
    </div>
    <div class="col-sm-3">
        <div class="form-group">
            {!! Form::label('sell_list_filter_variation_id', __('sale.product') . ':') !!}
            {!! Form::select('sell_list_filter_variation_id', $products, null, ['placeholder' => __('messages.all'), 'class' => 'form-control', 'id' => 'sell_list_filter_variation_id', 'style' => 'width:100%']); !!}
        </div>
    </div>
@endif
