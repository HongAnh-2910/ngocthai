    ...
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title no-print">
                @lang( 'purchase.approve_total_remaining' )
                (
                            ...
                            <tr {{ $is_pending ? 'style=color:#F00;' : '' }}>
                              ...
                              <td class="no-print" style="display: flex;">
                                  @if((auth()->user()->can('purchase.payments') && (in_array($transaction->type, ['purchase', 'purchase_return']) )) || (auth()->user()->can('sell.payments') && (in_array($transaction->type, ['sell', 'sell_return']))) || auth()->user()->can('expense.access') )
                                      ...
                                      @endif
                                      &nbsp;
                                      @if($payment->method == 'bank_transfer' && auth()->user()->can('sell.confirm_bank_transfer_method') && $payment->approval_status == 'pending')
                                          <button type="button" class="btn btn-danger btn-xs reject_remaining_payment" data-href="{{action('TransactionPaymentController@rejectPayment', [$payment->id]) }}" title="@lang('messages.confirm')">
