	            ...
	            <tbody>
	            	@foreach($product_stock_details as $product)
	            		<tr>
	            			...
	            			<td>
                        		<span data-currency_symbol=false >{{ number_format($product->stock, 2, '.', ',') ?? 0 }}</span> {{$product->unit}}
                        	</td>
                        	<td>
                        		<span class="display_currency" data-currency_symbol=true >{{ $product->unit_price * $product->stock }}</span>
                        	</td>
                        	<td>
                        		<span data-currency_symbol=false >{{ number_format($product->total_sold) ?? 0 }}</span> {{$product->unit}}
                        	</td>
                        	<td>
                        		<span data-currency_symbol=false >{{ number_format($product->total_transfered) ?? 0 }}</span> {{$product->unit}}
                        	</td>
                        	<td>
                        		<span data-currency_symbol=false >{{ number_format($product->total_adjusted) ?? 0 }}</span> {{$product->unit}}
                        	</td>
	            		</tr>
	            	@endforeach
	            </tbody>
