        ...
        <table class="table table-condensed table-bordered blue-header variation_value_table">
            ...
            @forelse ($product_variation->variations as $variation)
                ...
                <tr>
                    ...
                    <td class="{{$class}}">
                        {!! Form::label($array_name . '_' . $row_index . '_' . $variation_array_name . '_'.$variation_row_index.'_sell_price_inc_tax', __('product.default_selling_price') .':') !!}
{{--                        {!! Form::text($array_name . '[' . $row_index .'][' . $variation_array_name . '][' . $variation_row_index . '][default_sell_price]', @num_format($variation->default_sell_price), ['class' => 'form-control input-sm variable_dsp input_number', 'placeholder' => __('product.exc_of_tax'), 'required']); !!}--}}
                        {!! Form::text($array_name . '[' . $row_index .'][' . $variation_array_name . '][' . $variation_row_index . '][sell_price_inc_tax]', @num_format($variation->sell_price_inc_tax), ['class' => 'form-control input-sm variable_dsp_inc_tax input_number', 'id' => $array_name . '_' . $row_index . '_' . $variation_array_name . '_'.$variation_row_index.'_sell_price_inc_tax', 'required']); !!}

                        @foreach($price_groups[$variation->id] as $price_group)
                            <br>
                            {!! Form::label($array_name . '_' . $row_index . '_' . $variation_array_name . '_'.$variation_row_index.'_price_groups_'. $price_group->id, $price_group->name .':') !!}
                            {!! Form::text($array_name . '[' . $row_index . ']['.$variation_array_name.'][' . $variation_row_index . '][price_groups]['. $price_group->id .']', @number_format($price_group->price_inc_tax), ['class' => 'form-control input-sm input_number', 'id' => $array_name . '_' . $row_index . '_' . $variation_array_name . '_'.$variation_row_index.'_price_groups_'. $price_group->id]); !!}
                        @endforeach
                    </td>
                    <td>
                        @php
