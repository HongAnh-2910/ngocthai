...
{!! Form::open(['url' => action('ProductController@store'), 'method' => 'post',
    ...
    @component('components.widget', ['class' => 'box-primary'])
      ...
      <div class="clearfix"></div>
            ...
            {!! Form::label('unit_id', __('product.unit') . ':*') !!}
              <select class="form-control select2" name="unit_id" id="unit_id" required>
                  ...
                  @foreach($units as $unit)
                      <option value="{{ $unit->id }}" data-unit_type="{{ $unit->type }}">{{ $unit->actual_name }}</option>
                  @endforeach
              </select>
          </div>
        </div>
                ...
                <select name="sub_unit_ids[]" class="form-control select2" multiple id="sub_unit_ids">
                    @foreach($sub_units as $sub_unit_id => $sub_unit_value)
                        <option value="{{$sub_unit_id}}"
                            @if($sub_unit_id == $default_unit_id) locked="locked" selected
                            @endif
        ...
        <div class="col-sm-4" id="default_sub_unit_id_box">
            <div class="form-group">
                {!! Form::label('default_sub_unit_id', __('product.default_sub_unit') . ':*') !!}
                <select name="default_sub_unit_id" class="form-control" id="default_sub_unit_id" required>
                    <option value="">@lang('messages.please_select')</option>
                    @foreach($sub_units_except_base as $sub_unit_id => $sub_unit_value)
                        <option value="{{$sub_unit_id}}">{{$sub_unit_value['name']}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="col-sm-4" id="thickness_box">
            <div class="form-group">
                {!! Form::label('thickness',  __('lang_v1.thickness') . ':') !!}
                {!! Form::text('thickness', !empty($duplicate_product->thickness) ? $duplicate_product->thickness : null, ['class' => 'form-control', 'placeholder' => __('lang_v1.thickness')]); !!}
            </div>
        </div>
        <div class="col-sm-4" id="weight_box">
            <div class="form-group">
                {!! Form::label('weight',  __('lang_v1.m_to_kg') . ':') !!}
                {!! Form::text('weight', !empty($duplicate_product->weight) ? $duplicate_product->weight : null, ['class' => 'form-control', 'placeholder' => __('lang_v1.m_to_kg')]); !!}
            </div>
        </div>
        ...
        <div class="col-sm-4">
          <div class="form-group">
            {!! Form::label('product_locations', __('business.business_locations') . ':') !!} @show_tooltip(__('lang_v1.product_location_help'))
              {!! Form::select('product_locations[]', $business_locations, $all_location, ['class' => 'form-control select2', 'multiple', 'id' => 'product_locations', 'required']); !!}
          </div>
        </div>

        <div class="col-sm-4" id="enable_stock_box">
          <div class="form-group">
          <br>
            <label>
        ...
        <div class="col-sm-4 @if(!empty($duplicate_product) && $duplicate_product->enable_stock == 0) hide @endif" id="alert_quantity_div">
          <div class="form-group">
            ...
            {!! Form::text('alert_quantity', !empty($duplicate_product->alert_quantity) ? $duplicate_product->alert_quantity : null , ['class' => 'form-control',
    ...
    @component('components.widget', ['class' => 'box-primary'])
        ...
        {!! Form::hidden('enable_sr_no', false) !!}
        {{--<div class="col-sm-4">
          <div class="form-group">
            <br>
            <label>
