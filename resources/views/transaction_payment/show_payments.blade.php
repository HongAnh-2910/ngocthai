            ...
            @if($transaction->payment_status != 'paid')
                <div class="row">
                    <div class="col-md-12" style="margin-bottom: 10px;">
                        @if(!$can_not_create_payment && ((auth()->user()->can('purchase.payments') && (in_array($transaction->type, ['purchase', 'purchase_return']) )) || (auth()->user()->can('sell.payments') && (in_array($transaction->type, ['sell', 'sell_return']))) || (auth()->user()->can('expense.access'))))
                    ...
                    <div class="table-responsive">
                        ...
                        @forelse ($payments as $payment)
                            <tr>
                              ...
                              <td>{{ __('sale.'.$payment->type) }}</td>
                              <td>{{ $approval_statuses[$payment->approval_status] }}</td>
                              ...
                              @endif
                                @php
                                  ...
                                  $user_confirm = $transactionUtil->getUserConfirmPayment($payment->cashier_confirmed_id, $payment->admin_confirmed_id);
                                @endphp
                                <td>
                                    @if($user_confirm['type'] == 'both')
                                        @lang('sale.cashier_confirm'): {{ $user_confirm['cashier_name'] }}<br>
                                        @lang('sale.admin_confirm'): {{ $user_confirm['admin_name'] }}
                              ...
                              @if(!$can_not_approval_payment && auth()->user()->can('sell.confirm_bank_transfer_method') && ((auth()->user()->can('purchase.payments') && (in_array($transaction->type, ['purchase', 'purchase_return']) )) || (auth()->user()->can('sell.payments') && (in_array($transaction->type, ['sell', 'sell_return']))) || auth()->user()->can('sell.receipt_expense')))
