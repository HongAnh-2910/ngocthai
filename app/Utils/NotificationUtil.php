...
use App\DeviceToken;
use App\Notifications\ApprovalSellReturnNotification;
...
class NotificationUtil extends Util
{
    ...
    public function transactionPaymentNotification($payment)
    {
        $admin_users = User::role('Admin#1')->get();

        if($payment->method == 'bank_transfer' && $payment->approval_status == 'pending'){
            $other_users = User::permission('sell.confirm_bank_transfer_method')->get();
        }else{
            $confirm_bank_transfer_users = User::permission('sell.confirm_bank_transfer_method')->get();
            $confirm_cash_users = User::permission('sell.accept_received_money_to_custom')->get();
            $other_users = $confirm_bank_transfer_users->merge($confirm_cash_users);
        }

        $users = $admin_users->merge($other_users);

        foreach ($users as $user){
            dispatch(new SendNotifyToWeb($user, $payment));
        }
    }

    public function approvalSellReturnNotification($sellReturn)
    {
        $admin = User::role('Admin#1')->get();

        $approvalUser = User::permission('approval_sell_return')->get();
        $notifyUser = $admin->merge($approvalUser);

        $notifyUser->notify(new ApprovalSellReturnNotification($sellReturn));
    }

    public function getTransactionPayment($notification){
        $data = $notification->data;
        $util = new Util();

        $confirm_payment_types = $util->confirmPaymentTypes();
        if(isset($confirm_payment_types[$data['type']])){
            $confirm_payment_type = $confirm_payment_types[$data['type']];
        }else{
        ...
        if($data['approval_status'] == 'approved'){
            $msg = __(
                'sale.payment_confirmed_message',
                [
            ...
            $icon_class = 'fas fa-check bg-green';
        }elseif($data['approval_status'] == 'reject'){
            $msg = __(
                'sale.payment_reject_message',
                [
                    'payment_type' => $confirm_payment_type,
                    'invoice_no' => !empty($data['invoice_no']) ? $data['invoice_no'] : '',
                    'amount' => number_format($amount)
            ...
            $icon_class = 'fas fa-power-off bg-red';
        }else{
            $msg = __(
                'sale.payment_unconfirmed_message',
                [
                    'payment_type' => $confirm_payment_type,
                    'invoice_no' => !empty($data['invoice_no']) ? $data['invoice_no'] : '',
                    'amount' => number_format($amount)
