...
class Util
{
    ...
    */
    ...
    {
        return [ 'approved' => __('sale.approved'), 'pending' => __('sale.pending'), 'unapproved' => __('sale.unapproved'), 'reject' => __('sale.reject')];
    }

    public function newApprovalStatuses()
    {
        return [ 'approved' => __('sale.approved'), 'pending' => __('sale.pending'), 'unapproved' => __('sale.unapproved')];
    }

    public function approvalStatusColors()
    {
        return [ 'approved' => 'btn-success', 'pending' => 'bg-yellow', 'unapproved' => 'bg-red', 'reject' => 'bg-grey'];
    }
     ...
     * @return array
     */
    public function payment_types($location = null, $show_other = false)
    {
        $payment_types = ['cash' => __('lang_v1.cash'), 'bank_transfer' => __('lang_v1.bank_transfer')];
    ...
    public function setAndGetReferenceCount($type, $business_id = null)
    {
        ...
        $ref = ReferenceCount::where('ref_type', $type)->where('business_id', $business_id)->lockForUpdate()->first();
    ...
    * @return array
    */
    public function getSubUnits($business_id, $unit_id, $return_main_unit_if_empty = false, $product_id = null, $except_base = false, $include_weight_unit_if_is_area = false)
    {
        $unit = Unit::where('business_id', $business_id);

        if ($except_base) {
            $unit = $unit->with(['sub_units' => function($query){
        ...
        if($unit->base_unit_id){
            $base_unit = Unit::find($unit->base_unit_id);
            $sub_units[$base_unit->id] = [
                'id' => $base_unit->id,
                'name' => $base_unit->actual_name,
                'multiplier' => $base_unit->base_unit_multiplier,
                'allow_decimal' => $base_unit->allow_decimal,
                'width' => $unit->width,
                'height' => $unit->height,
                'type' => $base_unit->type,
                'is_default' => $base_unit->is_default
            ];
        }
            ...
            foreach ($unit->sub_units as $sub_unit) {
                ...
                if (empty($related_sub_units) || in_array($sub_unit->id, $related_sub_units)) {
                    $sub_units[$sub_unit->id] = [
                        ...
                        'width' => $sub_unit->width,
                        'height' => $sub_unit->height,
                        'type' => $sub_unit->type,
                        'is_default' => $sub_unit->is_default
                    ];
                }
            }
        }

        if($include_weight_unit_if_is_area && $unit->type == 'area'){
            $weight_units = Unit::where('type', 'weight')
                ->orderBy('base_unit_id', 'asc')
                ->get();

            foreach($weight_units as $weight_unit){
                $sub_units[$weight_unit->id] = [
                    'name' => $weight_unit->actual_name,
                    'multiplier' => $weight_unit->base_unit_multiplier,
                    'allow_decimal' => $weight_unit->allow_decimal,
                    'width' => $weight_unit->width,
                    'height' => $weight_unit->height,
                    'type' => $weight_unit->type,
                    'is_default' => $weight_unit->is_default
                ];
            }
        }
...
//            'cancelled' => __('restaurant.cancelled')
            'not_shipped' => __('lang_v1.not_shipped'),
            'shipped' => __('lang_v1.shipped'),
        ];
    ...
    public function reduce_debt_statuses()
    {
        $statuses = [
            'pending' => __('sale.pending'),
            'final' => __('sale.approved'),
        ];

        return $statuses;
    }

    public function export_statuses()
    {
        $statuses = [
            'none' => __('sale.not_yet_exported'),
            'pending' => __('sale.pending'),
            'approved' => __('sale.delivered'),
        ];
    ...
    {
        $target_types = [
            'amount' => __('target.type_amount'),
            'product' => __('target.type_product'),
            'category' => __('target.type_category'),
            'profit' => __('target.type_profit')
        ];

        return $target_types;
    }

    public function confirmPaymentTypes()
    {
        return [ 'deposit' => __('sale.notify_deposit'), 'cod' => __('sale.notify_cod'), 'return' => __('sale.notify_return'), 'normal' => __('sale.notify_normal'), 'receipt' => __('sale.notify_receipt'), 'expense' => __('sale.notify_expense')];
    }

    public function unitTypes()
    {
        return [
            'area' => __('unit.area_type'),
            'pcs' => __('unit.pcs_type'),
            'meter' => __('unit.meter_type'),
            'service' => __('unit.service_type'),
//            'weight' => __('unit.weight_type'),
        ];
    }

    public function return_price_types()
    {
        return [
            'old' => __('purchase.by_old_sell_price'),
            'new' => __('purchase.by_new_sell_price'),
            'new_by_plate' => __('purchase.by_new_origin_plate')
        ];
    }
}
