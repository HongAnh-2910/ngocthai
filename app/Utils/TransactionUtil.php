...
//            'tax_id' => !empty($input['tax_rate_id']) ? $input['tax_rate_id'] : null,
            ...
            'tax_amount' => str_replace(',', '', $input['order_tax_text']),
            ...
            'is_approved_by_cashier' => 0,
            'is_approved_by_storekeeper' => 0,
            'is_approved_by_admin' => 0,
            'phone_contact' => !empty($input['phone_contact']) ? $input['phone_contact'] : null,
            'is_service_order' => !empty($input['is_service_order']) ? $input['is_service_order'] : 0,
        ]);
...
//                $uf_unit_price = $uf_data ? $this->num_uf($product['unit_price']) : $product['unit_price'];
                ...
                $quantity = !empty($product['quantity']) ? $this->num_uf($product['quantity']) : 1;
                $width = !empty($product['width']) ? $this->num_uf($product['width']) : 1;
                $height = !empty($product['height']) ? $this->num_uf($product['height']) : 1;
...
//                    'tax_id' => $product['tax_id'],
                    ...
                    'sub_unit_id' => isset($product['sub_unit_id']) ? $product['sub_unit_id'] : 1,
    ...
    public function editSellLine($product, $location_id, $status_before, $multiplier = 1)
    {
        ...
        $sell_line->fill([
            'product_id' => $product['product_id'],
            'variation_id' => $product['variation_id'],
            'quantity' => $area * $this->num_uf($product['quantity']),
            'quantity_line' => $this->num_uf($product['quantity']),
            'height' => $this->num_uf($product['height']),
            'width' => $this->num_uf($product['width']),
            'unit_price_before_discount' => $unit_price_before_discount,
            'unit_price' => $unit_price,
            'line_discount_type' => !empty($product['line_discount_type']) ? $product['line_discount_type'] : null,
            'line_discount_amount' => !empty($product['line_discount_amount']) ? $this->num_uf($product['line_discount_amount']) : 0,
//            'item_tax' => $this->num_uf($product['item_tax']) / $area,
    ...
    public function createOrUpdatePaymentLines($transaction, $payments, $business_id = null, $user_id = null, $uf_data = true)
    {
        ...
        foreach ($payments as $payment) {
            ...
            if (!empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    $payment_data = [
                        'amount' => !empty($payment_amount) ? $payment_amount : 0,
                        ...
                        'type' => isset($payment['type']) ? $payment['type'] : 'normal',
                        'approval_status' => isset($payment['approval_status']) ? $payment['approval_status'] : 'unapproved',
                        'account_id' => !empty($payment['account_id']) ? $payment['account_id'] : null,
                        'cashier_confirmed_id' => isset($payment['cashier_confirmed_id']) ? $payment['cashier_confirmed_id'] : null,
                    ];
        ...
        if (!empty($payments_formatted)) {
            $transaction_payments = $transaction->payment_lines()->saveMany($payments_formatted);
            foreach($transaction_payments as $transaction_payment){
                if ($transaction->status != 'draft') {
                    $notificationUtil = new NotificationUtil();
                    $notificationUtil->transactionPaymentNotification($transaction_payment);
                }
            }
...
//        }
     ...
     * @param boolean $addVat
     *
     * @return array
     */
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type, $addVat = false)
    {
        $il = $invoice_layout;

        $transaction = Transaction::with('return_parent')
            ->where('transactions.id', $transaction_id)
            ->select([
                '*',
                DB::raw("(SELECT CONCAT(COALESCE(users.first_name, ''),' ',COALESCE(users.last_name,'')) FROM users WHERE users.id = transactions.created_by LIMIT 1) as added_by"),
                DB::raw('(SELECT IF(TP.is_return = 1,-1*TP.amount,TP.amount) FROM transaction_payments AS TP WHERE
                        TP.transaction_id=transactions.id AND TP.type = "deposit" ORDER BY TP.id LIMIT 1) as deposit'),
                DB::raw('(SELECT IF(TP.is_return = 1,-1*TP.amount,TP.amount) FROM transaction_payments AS TP WHERE
                        TP.transaction_id=transactions.id AND TP.type = "cod" ORDER BY TP.id LIMIT 1) as cod'),
            ])
            ->first();
        $transaction_type = $transaction->type;

        $transaction_return = Transaction::find($transaction->return_parent_id);

        $sell_return = TransactionPlateLinesReturn::where('transaction_id', $transaction->return_parent_id)->get();
        ...
        $output = [
            ...
            'table_unit_price_label' => __('lang_v1.unit_cost'),
            'table_area_label' => __('lang_v1.table_area_label'),
            'table_subtotal_label' => $il->table_subtotal_label,
            'table_sell_return_details' => __('lang_v1.label_sell_return_details'),
            'table_return_subtotal' => __('lang_v1.return_subtotal'),
            'table_return_discount' => __('lang_v1.return_discount'),
            'table_net_total_amount_return' => __('lang_v1.net_total_amount_return'),
        ...
        $output['is_visitor'] = $transaction->contact->is_default;
        $output['delivered_to'] = $transaction_type == 'sell_return' ? $transaction_return->delivered_to : $transaction->delivered_to;

        $output['shipping_note'] = '';
        if($transaction_type == 'sell_return'){
            if(!empty($transaction_return->shipping_details)){
                $output['shipping_note'] .= $transaction_return->shipping_details;
            }
            if(!empty($transaction_return->phone_contact)){
                if(!empty($output['shipping_note'])){
                    $output['shipping_note'] .= ' ';
                }
                $output['shipping_note'] .= $transaction_return->phone_contact;
            }
        }else{
            if(!empty($transaction->shipping_details)){
                $output['shipping_note'] .= $transaction->shipping_details;
            }
            if(!empty($transaction->phone_contact)){
                if(!empty($output['shipping_note'])){
                    $output['shipping_note'] .= ' ';
                }
                $output['shipping_note'] .= $transaction->phone_contact;
            }
        }

        $output['shipping_address'] = $transaction_type == 'sell_return' ? $transaction_return->shipping_address : $transaction->shipping_address;
...
//        $output['shipping_address'] = !empty($transaction->shipping_address()) ? $transaction->shipping_address() : $transaction->shipping_address;
        ...
        $output['invoice_day'] = \Carbon::parse($transaction->transaction_date)->format('d');
        $output['invoice_month'] = \Carbon::parse($transaction->transaction_date)->format('m');
        $output['invoice_year'] = \Carbon::parse($transaction->transaction_date)->format('Y');
        $output['invoice_year'] = substr($output['invoice_year'], -1);
        ...
        if ($transaction_type == 'sell') {
            ...
            foreach ($details['lines'] as $line) {
                if (!empty($line['group_tax_details'])) {
                    foreach ($line['group_tax_details'] as $tax_group_detail) {
                        ...
                        $output['taxes'][$tax_group_detail['name']] += round_int($tax_group_detail['calculated_tax'] * $line['quantity_uf'], env('DIGIT', 4));
                    }
                    ...
                    $output['taxes'][$line['tax_name']] += round_int($line['tax_unformatted'] * $line['quantity_uf'],  env('DIGIT', 4));
                }

                $total_quantity += round_int($line['quantity_uf'], env('DIGIT', 4));
            }

            if (!empty($il->common_settings['total_quantity_label'])) {
                $output['total_quantity_label'] = $il->common_settings['total_quantity_label'];
                $output['total_quantity'] = $this->num_f(round_int($total_quantity, env('DIGIT', 4)), false, $business_details, true);
            }
        } elseif ($transaction_type == 'sell_return') {
            $business_id = request()->session()->get('user.business_id');
            $sell_return_lines = Transaction::where('business_id', $business_id)
                ->where('id', $transaction->return_parent_id)
                ->with(
                    'contact',
                    'return_parent',
                    'tax',
                    'sell_lines.plate_line_return.warehouse',
                    'sell_lines.product',
            ...
            foreach ($sell_return_lines->sell_lines as $key => $value) {
                if (!empty($value->sub_unit_id)) {
                    $formated_sell_line = $this->recalculateSellLineTotals($business_id, $value);
                    $sell_return_lines->sell_lines[$key] = $formated_sell_line;
                }
            }
            ...
            $details = $this->_receiptDetailsSellReturnLines($sell_return_lines, $transaction, $il, $business_details);
            $output['lines'] = $details['lines'];
        ...
        $output['subtotal'] = ($transaction->total_before_tax != 0) ? $this->num_f(round_int($transaction->total_before_tax, env('DIGIT', 4)), $show_currency, $business_details) : 0;
...
//        $output['discount_amount'] = $transaction->discount_amount > 0 ? $this->num_f(round_int($transaction->discount_amount, env('DIGIT', 4)), false, $business_details) : '';
        $output['discount_amount'] = $transaction->discount_amount > 0 ? $this->num_f($transaction->discount_amount, $show_currency = false, $business_details) : '';


        /*if ($transaction->discount_type == 'percentage') {
            $return_discount = $transaction->final_total * ($transaction->discount_amount / 100);
        } else {
            $return_discount = $transaction->discount_amount;
        }*/
//        $output['return_discount'] = $return_discount;
        $output['final_total'] = round_int($transaction->final_total, env('DIGIT', 4));
        $output['total_before_tax'] = round_int($transaction->total_before_tax, env('DIGIT', 4));
        ...
        $output['shop_return_amount'] = round_int($transaction->shop_return_amount, env('DIGIT', 4));
        $output['total_money_return'] = $transaction->final_total;
        $output['discount'] = ($discount != 0) ? $this->num_f(round_int($discount, env('DIGIT', 4)), false, $business_details) : 0;

        //reward points
        if ($business_details->enable_rp == 1 && !empty($transaction->rp_redeemed)) {
            $output['reward_point_label'] = $business_details->rp_name;
            $output['reward_point_amount'] = $this->num_f(round_int($transaction->rp_redeemed_amount, env('DIGIT', 4)), $show_currency, $business_details);
        }

        //Format tax
        if (!empty($output['taxes'])) {
            foreach ($output['taxes'] as $key => $value) {
                $output['taxes'][$key] = $this->num_f(round_int($value, env('DIGIT', 4)), $show_currency, $business_details);
            }
        }
        ...
        $output['tax_label'] .= ':';
        $output['tax'] = ($transaction->tax_amount != 0) ? $this->num_f(round_int($transaction->tax_amount, env('DIGIT', 4)), $show_currency, $business_details) : 0;
        ...
        if ($transaction->tax_amount != 0 && $tax && $tax->is_tax_group) {
            ...
            foreach ($transaction_group_tax_details as $value) {
                $output['group_tax_details'][$value['name']] = $this->num_f(round_int($value['calculated_tax'], env('DIGIT', 4)), $show_currency, $business_details);
            }
        }

        //Shipping charges
        $output['shipping_charges'] = ($transaction->shipping_charges != 0) ? $this->num_f(round_int($transaction->shipping_charges, env('DIGIT', 4)), $show_currency, $business_details) : 0;
        ...
        $cod = $transaction->cod;
        $output['deposit'] = ($transaction->deposit != 0) ? $this->num_f(round_int($transaction->deposit, env('DIGIT', 4)), $show_currency = false, $business_details) : 0;
        $output['deposit_label'] = trans("lang_v1.deposit").':';
        $output['cod'] = ($transaction->cod != 0) ? $this->num_f(round_int($transaction->cod, env('DIGIT', 4)), $show_currency = false, $business_details) : 0;
        $output['cod_label'] = trans("lang_v1.cod").':';

        //Total
        if ($transaction_type == 'sell_return') {
            $output['total_label'] = $invoice_layout->cn_amount_label . ':';
            $output['total'] = $this->num_f(round_int($transaction->final_total, env('DIGIT', 4)), $show_currency = false, $business_details);
        } else {
            $output['total_label'] = $invoice_layout->total_label . ':';
            $total_amount_product = 0;
            foreach ($details['lines'] as $line) {
                $total_amount_product += $this->productUtil->num_uf($line['line_total']);
            }
            $final_total = $total_amount_product - $transaction->discount_amount + $transaction->tax_amount + $transaction->vat_money + $transaction->shipping_charges;
            $output['total'] = $this->num_f(round_int($final_total, env('DIGIT', 4)), $show_currency = false, $business_details);

//            $total_exc_vat = $transaction->final_total - $transaction->vat_money;
//            $output['total'] = $this->num_f(round_int($total_exc_vat, env('DIGIT', 4)), $show_currency = false, $business_details);
        }

        //VAT money
        if ($addVat) {
//            $payment = TransactionPayment::query()->where('transaction_id', $transaction_id)->where('approval_status', TransactionPayment::APPROVED)->sum('amount');
            $output['vat_money'] = ($transaction->vat_money != 0) ? $this->num_f(round_int($transaction->vat_money, env('DIGIT', 4)), $show_currency, $business_details) : 0;
            $output['vat_label'] = trans("sale.vat").':';
            $output['total'] = $this->num_f(round_int($transaction->shipping_charges + $transaction->vat_money, env('DIGIT', 4)), $show_currency, $business_details);
            $output['total_label'] = $invoice_layout->total_label . ':';
        }

        $output['remaining_label'] = trans("lang_v1.return_money") . ':';
        $remaining = ($transaction->final_total - $transaction->vat_money) - ($deposit != 0 ? $deposit : 0) - ($cod != 0 ? $cod : 0);
        $output['remaining'] = $remaining >= 0 ? $this->num_f(round_int($remaining, env('DIGIT', 4)), $show_currency = false, $business_details) : '(' . $this->num_f(round_int(abs($remaining), env('DIGIT', 4)), $show_currency = false, $business_details) . ')';
        ...
        if ($transaction_type == 'sell' && $transaction->status == 'final') {
            ...
            $output['total_paid'] = ($paid_amount == 0) ? 0 : $this->num_f(round_int($paid_amount, env('DIGIT', 4)), $show_currency, $business_details);
            $output['total_paid_label'] = $il->paid_label;
            $output['total_due'] = ($due == 0) ? 0 : $this->num_f(round_int($due, env('DIGIT', 4)), $show_currency, $business_details);
            ...
            if ($il->show_previous_bal == 1) {
                ...
                if (!empty($all_due)) {
                    ...
                    $output['all_due'] = $this->num_f(round_int($all_due, env('DIGIT', 4)), $show_currency, $business_details);
                }
            }
                        ...
                        if ($value['method'] == 'cash') {
                            $output['payments'][] =
                                ['method' => $method . ($value['is_return'] == 1 ? ' (' . $il->change_return_label . ')(-)' : ''),
                                    'amount' => $this->num_f(round_int($value['amount'], env('DIGIT', 4)), $show_currency, $business_details),
    ...
    public function getDepositReceiptDetails($transaction_id)
    {
        ...
        $address = implode(", ", array_filter([$transaction->contact->landmark, $transaction->contact->city, $transaction->contact->state]));
        ...
        $output = [
            ...
            'invoice_day' => Carbon::parse($transaction->transaction_date)->format('d'),
            'invoice_month' => Carbon::parse($transaction->transaction_date)->format('d'),
            'invoice_year' => substr(Carbon::parse($transaction->transaction_date)->format('Y'), -1),
            'added_by' => $transaction->added_by,
            'is_visitor' => $transaction->contact->is_default,
        ];

        return (object) $output;
    }

    public function getDeliverReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type, $print_new_template)
    {
        $il = $invoice_layout;

        $transaction = Transaction::with('sell_lines.sub_unit')->where('transactions.id', $transaction_id)
            ->select([
                '*',
                DB::raw("(SELECT CONCAT(COALESCE(users.first_name, ''),' ',COALESCE(users.last_name,'')) FROM users WHERE users.id = transactions.created_by LIMIT 1) as added_by"),
            ])
        ...
        $output = [
            ...
            'table_net_total_amount_return' => __('lang_v1.net_total_amount_return'),
            'added_by' => $transaction->added_by,
            'table_plate_quantity_before_label' => __('sale.plate_quantity_before'),
            'is_new_template' => $transaction->plates_sort_order ? true : false,
            'print_new_template' => $print_new_template,
        ];
        ...
        $output['contact'] = '';
        if ($il->show_mobile_number == 1 && !empty($location_details->mobile)) {
            $output['contact'] .= __('lang_v1.contact_no') . ': ' . $location_details->mobile;
        }
        if ($il->show_alternate_number == 1 && !empty($location_details->alternate_number)) {
            if (empty($output['contact'])) {
                $output['contact'] .= __('lang_v1.contact_no') . ': ' . $location_details->alternate_number;
            } else {
                $output['contact'] .= ', ' . $location_details->alternate_number;
            }
        }
        ...
        $output['shipping_address'] = $transaction->shipping_address;
        ...
        if ($il->show_customer == 1) {
            $output['customer_label'] = !empty($il->customer_label) ? $il->customer_label : '';
            $output['customer_name'] = !empty($customer->name) ? $customer->name: '';
            $output['customer_address'] = implode(',', array_filter([$customer->city, $customer->state, $customer->country]));
        ...
        if ($transaction_type == 'sell') {
            $sell_line_relations = [
                ...
                'sub_unit',
                'warranties',
                'plate_lines.selected_plate_stock.warehouse',
                'plate_lines.selected_plate_stock.product.unit'
            ];
            ...
            $details = $this->_deliverReceiptDetailsSellLines($lines, $il, $business_details);

            // lines for old print template stock deliver
            $output['lines'] = $details['lines'];
            $output['taxes'] = [];
            $total_quantity = 0;
            foreach ($details['lines'] as $line) {
                $order_line = $line['order'];
                if (!empty($order_line['group_tax_details'])) {
                    foreach ($order_line['group_tax_details'] as $tax_group_detail) {
                        if (!isset($output['taxes'][$tax_group_detail['name']])) {
                            $output['taxes'][$tax_group_detail['name']] = 0;
                        }
                        $output['taxes'][$tax_group_detail['name']] += ($tax_group_detail['calculated_tax'] * $order_line['quantity_uf']);
                    }
                } elseif (!empty($order_line['tax_unformatted']) && $order_line['tax_unformatted'] != 0) {
                    if (!isset($output['taxes'][$order_line['tax_name']])) {
                        $output['taxes'][$order_line['tax_name']] = 0;
                    }

                    $output['taxes'][$order_line['tax_name']] += ($order_line['tax_unformatted'] * $order_line['quantity_uf']);
                }

                $total_quantity += $order_line['quantity_uf'];
            }

            // new_lines for new print template stock deliver
            if($output['is_new_template']){
                $new_lines = json_decode($transaction->plates_sort_order, true);
                $output['new_lines'] = $this->_newDeliverReceiptDetailsSellLines($new_lines);
            }
        ...
        //Subtotal
//        $output['subtotal_label'] = $il->sub_total_label . ':';
        $output['subtotal_label'] = __('purchase.line_total') . ':';
        ...
        $output['discount_label'] .= ':';
        $output['discount_type'] = $transaction->discount_type;
        $output['discount_amount'] = $transaction->discount_amount > 0 ? $this->num_f($transaction->discount_amount, $show_currency, $business_details) : '';
        ...
        $output['return_discount'] = $return_discount;
        $output['final_total'] = $transaction->final_total;
        $output['total_money_return'] = $transaction->final_total - $return_discount;
...
//                                ];
    ...
    public function getShippingReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type, $addVat = false)
    {
        $il = $invoice_layout;
        ...
        $transaction = Transaction::where('transactions.id', $transaction_id)
            ...
            ->select([
                ...
                DB::raw('(SELECT IF(TP.is_return = 1,-1*TP.amount,TP.amount) FROM transaction_payments AS TP WHERE
                        TP.transaction_id=transactions.id AND TP.type = "cod" ORDER BY TP.id LIMIT 1) as cod'),
                DB::raw("CONCAT(ss.first_name,' ',ss.last_name) AS shipper"),
            ])
            ->first();
        $transaction_type = $transaction->type;
        ...
        if ($il->show_email == 1 && !empty($location_details->email)) {
            if (!empty($output['contact'])) {
                $output['email'] .= __('business.email') . ': ' . $location_details->email;
            }
        }
        ...
        if ($transaction_type == 'sell_return') {
            ...
            $output['invoice_no_prefix'] = $il->invoice_no_prefix;
            if (strpos($il->invoice_no_prefix, '.') !== false) {
                $invoice_no_prefix = str_replace('.', '', $il->invoice_no_prefix);
                $output['invoice_no_prefix'] = $invoice_no_prefix;
            }
            $output['invoice_heading'] = $il->invoice_heading;
            if ($transaction->payment_status == 'paid' && !empty($il->invoice_heading_paid)) {
                $output['invoice_heading'] .= ' ' . $il->invoice_heading_paid;
...
//        $output['return_discount'] = $return_discount;
        ...
        $output['total_money_return'] = round_int($transaction->total_before_tax - $discount, env('DIGIT', 4));
        $output['discount'] = ($discount != 0) ? $this->num_f(round_int($discount, env('DIGIT', 4)), false, $business_details) : 0;
...
//            $unit_name = !empty($unit->short_name) ? $unit->short_name : '';
            ...
            $unit_name = $line->sub_unit->actual_name;
            $is_default_unit = $line->sub_unit->is_default;

            $unit_type = $line->sub_unit->type;
            $unit_multiplier = $line->sub_unit->base_unit_multiplier;
            if (!$unit_multiplier) {
                $unit_multiplier = 1;
            }
            $unit_weight = $line->product->weight;
//            $base_unit_id = $line->sub_unit->base_unit_id;
            $area = $unit_type == 'weight' ? ($line->quantity_line * $unit_multiplier) / $unit_weight : $line->quantity_line * $line->width * $line->height;
            $unit_price_before_discount = $unit_type == 'weight' ? $line->unit_price_before_discount * $area : $line->unit_price_before_discount;
            $unit_price_after_discount = $unit_price_before_discount - $line->line_discount_amount;
            $line_total = $unit_type == 'weight' ? $unit_price_after_discount * $line->quantity_line : ($is_default_unit == 0 ? $unit_price_after_discount * $line->quantity_line : $unit_price_after_discount * $area);
...
//                'base_unit_id' => $base_unit_id,
                ...
                'unit_price' => $this->num_f(round_int($line->unit_price, env('DIGIT', 4)), false, $business_details),
                'tax' => $this->num_f(round_int($line->item_tax, env('DIGIT', 4)), false, $business_details),
                ...
                'unit_price_inc_tax' => $this->num_f($unit_price_after_discount),
                'unit_price_exc_tax' => $this->num_f(round_int($line->unit_price, env('DIGIT', 4)), false, $business_details),
                'price_exc_tax' => $this->num_f(round_int($line->quantity * $line->unit_price, env('DIGIT', 4))),
                'unit_price_before_discount' => $this->num_f(round_int($line->unit_price_before_discount, env('DIGIT', 4)), false, $business_details),

                //Fields for 4th column
                'line_total' => $this->num_f(round_int($line_total, env('DIGIT', 4)), false, $business_details),
                'quantity_line' => $line->quantity_line,
                'width' => $line->width,
                'height' => $line->height,
            ...
            $line_array['line_discount'] = method_exists($line, 'get_discount_amount') ? $this->num_f(round_int($line->get_discount_amount(), env('DIGIT', 4)), false, $business_details) : 0;
            ...
            if (!empty($line->modifiers)) {
                foreach ($line->modifiers as $modifier_line) {
                    ...
                    $modifier_line_array = [
                        ...
                        //Field for 3rd column
                        'unit_price_inc_tax' => $this->num_f(round_int($modifier_line->unit_price_inc_tax, env('DIGIT', 4)), false, $business_details),
                        'unit_price_exc_tax' => $this->num_f(round_int($modifier_line->unit_price, env('DIGIT', 4)), false, $business_details),
                        'price_exc_tax' => $modifier_line->quantity * $modifier_line->unit_price,

                        //Fields for 4th column
                        'line_total' => $this->num_f(round_int($modifier_line->unit_price * $line->quantity, env('DIGIT', 4)), false, $business_details),
                        'quantity_line' => $modifier_line->quantity_line
                    ];
    ...
    protected function _deliverReceiptDetailsSellLines($lines, $il, $business_details)
    {
        $is_lot_number_enabled = $business_details->enable_lot_number;
        $is_product_expiry_enabled = $business_details->enable_product_expiry;
...
//            $unit_name = !empty($unit->short_name) ? $unit->short_name : '';
            ...
            $order_line_array = [
                ...
                'quantity_line' => $this->num_f($line->quantity_line, false, $business_details),
                'width' => $line->width,
                'height' => $line->height,
            ];
            ...
            if (!empty($temp)) {
                $order_line_array['product_custom_fields'] = implode(',', $temp);
            }
            ...
            if (!empty($tax_details)) {
                if ($tax_details->is_tax_group) {
                    ...
                    $order_line_array['group_tax_details'] = $group_tax_details;
            ...
            $order_line_array['line_discount'] = method_exists($line, 'get_discount_amount') ? $this->num_f($line->get_discount_amount(), false, $business_details) : 0;
            if ($line->line_discount_type == 'percentage') {
                $order_line_array['line_discount'] .= ' (' . $this->num_f($line->line_discount_amount, false, $business_details) . '%)';
            }

            if ($il->show_brand == 1) {
                $order_line_array['brand'] = !empty($brand->name) ? $brand->name : '';
            }
            if ($il->show_sku == 1) {
                $order_line_array['sub_sku'] = !empty($variation->sub_sku) ? $variation->sub_sku : '' ;
            }
            if ($il->show_image == 1) {
                $media = $variation->media;
                if (count($media)) {
                    $first_img = $media->first();
                    $order_line_array['image'] = !empty($first_img->display_url) ? $first_img->display_url : asset('/img/default.png');
                } else {
                    $order_line_array['image'] = $product->image_url;
                }
            }
            if ($il->show_cat_code == 1) {
                $order_line_array['cat_code'] = !empty($cat->short_code) ? $cat->short_code : '';
            }
            if ($il->show_sale_description == 1) {
                $order_line_array['sell_line_note'] = !empty($line->sell_line_note) ? $line->sell_line_note : '';
            }
            if ($is_lot_number_enabled == 1 && $il->show_lot == 1) {
                $order_line_array['lot_number'] = !empty($line->lot_details->lot_number) ? $line->lot_details->lot_number : null;
                $order_line_array['lot_number_label'] = __('lang_v1.lot');
            }

            if ($is_product_expiry_enabled == 1 && $il->show_expiry == 1) {
                $order_line_array['product_expiry'] = !empty($line->lot_details->exp_date) ? $this->format_date($line->lot_details->exp_date, false, $business_details) : null;
                $order_line_array['product_expiry_label'] = __('lang_v1.expiry');
            }
            ...
            if ($is_warranty_enabled && !empty($line->warranties->first())) {
                $warranty = $line->warranties->first();
                if (!empty($il->common_settings['show_warranty_name'])) {
                    $order_line_array['warranty_name'] = $warranty->name;
                }
                if (!empty($il->common_settings['show_warranty_description'])) {
                    $order_line_array['warranty_description'] = $warranty->description;
                }
                if (!empty($il->common_settings['show_warranty_exp_date'])) {
                    $order_line_array['warranty_exp_date'] = $warranty->getEndDate($line->transaction->transaction_date);
                }
            }

            $line_array['order'] = $order_line_array;
            $line_array['deliver'] = [];

            foreach ($line->plate_lines as $plate_line){
                $selected_plate_stock = $plate_line->selected_plate_stock;
                $product = $selected_plate_stock->product;
                $variation = $selected_plate_stock->variation;
                $product_variation = $selected_plate_stock->variation->product_variation;
                $unit = $selected_plate_stock->product->unit;
                $brand = $selected_plate_stock->product->brand;
                $cat = $selected_plate_stock->product->category;
                $tax_details = TaxRate::find($line->tax_id);

                $remaining_widths = RemainingPlateLine::where('transaction_plate_line_id', $plate_line->id)
                    ->select(
                        'width',
                        'quantity'
                    )
                ...
                $deliver_line_array = [
                    ...
                    //Field for 2nd column
                    'quantity' => $this->num_f($plate_line->quantity, false, $business_details, true),
                    'quantity_uf' => $plate_line->quantity,
                    'units' => $unit->actual_name,
                    'unit_type' => $unit->type,
                    'height' => $plate_line->height,
                    'warehouse' => $selected_plate_stock->warehouse->name,
                    'height_before_cut' => $plate_line->selected_height,
                    'width_before_cut' => $plate_line->selected_width,
                    'width' => $plate_line->width,
                    ...
                    'quantity_before' => $plate_line->selected_quantity,
                    'remaining_widths' => $remaining_widths,
                    'note' => $plate_line->note,
                ];
                ...
                if (!empty($temp)) {
                    $deliver_line_array['product_custom_fields'] = implode(',', $temp);
                }
                ...
                if (!empty($tax_details)) {
                    if ($tax_details->is_tax_group) {
                        ...
                        $deliver_line_array['group_tax_details'] = $group_tax_details;
                    }
                }

                $deliver_line_array['line_discount'] = method_exists($line, 'get_discount_amount') ? $this->num_f($line->get_discount_amount(), false, $business_details) : 0;
                if ($line->line_discount_type == 'percentage') {
                    $deliver_line_array['line_discount'] .= ' (' . $this->num_f($line->line_discount_amount, false, $business_details) . '%)';
                }

                if ($il->show_brand == 1) {
                    $deliver_line_array['brand'] = !empty($brand->name) ? $brand->name : '';
                }
                if ($il->show_sku == 1) {
                    $deliver_line_array['sub_sku'] = !empty($variation->sub_sku) ? $variation->sub_sku : '' ;
                }
                if ($il->show_image == 1) {
                    $media = $variation->media;
                    if (count($media)) {
                        $first_img = $media->first();
                        $deliver_line_array['image'] = !empty($first_img->display_url) ? $first_img->display_url : asset('/img/default.png');
                    } else {
                        $deliver_line_array['image'] = $product->image_url;
                    }
                }
                if ($il->show_cat_code == 1) {
                    $deliver_line_array['cat_code'] = !empty($cat->short_code) ? $cat->short_code : '';
                }
                if ($il->show_sale_description == 1) {
                    $deliver_line_array['sell_line_note'] = !empty($line->sell_line_note) ? $line->sell_line_note : '';
                }
                if ($is_lot_number_enabled == 1 && $il->show_lot == 1) {
                    $deliver_line_array['lot_number'] = !empty($line->lot_details->lot_number) ? $line->lot_details->lot_number : null;
                    $deliver_line_array['lot_number_label'] = __('lang_v1.lot');
                }

                if ($is_product_expiry_enabled == 1 && $il->show_expiry == 1) {
                    $deliver_line_array['product_expiry'] = !empty($line->lot_details->exp_date) ? $this->format_date($line->lot_details->exp_date, false, $business_details) : null;
                    $deliver_line_array['product_expiry_label'] = __('lang_v1.expiry');
                }
                ...
                if ($is_warranty_enabled && !empty($line->warranties->first())) {
                    $warranty = $line->warranties->first();
                    if (!empty($il->common_settings['show_warranty_name'])) {
                        $deliver_line_array['warranty_name'] = $warranty->name;
                    }
                    if (!empty($il->common_settings['show_warranty_description'])) {
                        $deliver_line_array['warranty_description'] = $warranty->description;
                    }
                    if (!empty($il->common_settings['show_warranty_exp_date'])) {
                        $deliver_line_array['warranty_exp_date'] = $warranty->getEndDate($line->transaction->transaction_date);
                    }
                }

                $line_array['deliver'][] = $deliver_line_array;
            }
    ...
    protected function _newDeliverReceiptDetailsSellLines($lines)
    {
        $output_lines = [];
        foreach ($lines as $line) {
            $plate_stock = PlateStock::with([
                ...
                'variation',
                'warehouse',
            ])->find($line['plate_stock_id']);
            $product = $plate_stock->product;
            $variation = $plate_stock->variation;
            $unit = $plate_stock->product->unit;
            $unit_type = $unit->type;
            if($unit->type == 'pcs'){
                $unit_name = 'C';
            }else{
            ...
            $order_line_array = [
                'warehouse' => $plate_stock->warehouse->name,
                'name' => $product->name,
                'variation' => (empty($variation->name) || $variation->name == 'DUMMY') ? '' : $variation->name,
                'sku' => $product->sku,
                'selected_width' => $line['selected_width'],
                'selected_height' => $plate_stock->height,
                'selected_quantity' => $line['selected_quantity'],
                'remaining_width' => $line['remaining_width'],
            ...
            foreach ($line['deliver_plates'] as $deliver_plate){
                $deliver_line_array = [
                    'deliver_width' => $deliver_plate['deliver_width'],
                    'deliver_quantity' => $deliver_plate['deliver_quantity'],
                ];
     ...
     * @return array
     */
    protected function _receiptDetailsSellReturnLines($lines, $transaction, $il, $business_details)
    {
        $is_lot_number_enabled = $business_details->enable_lot_number;
        $is_product_expiry_enabled = $business_details->enable_product_expiry;
        ...
        $output_lines = [];
        $output_taxes = ['taxes' => []];
        foreach ($lines->sell_lines as $line) {
            $product = $line->product;
            $variation = $line->variations;
            $unit = $line->product->unit;
            $brand = $line->product->brand;
...
//                'tax' => $this->num_f($line->item_tax, false, $business_details),
                ...
                //Field for 3rd column
                'unit_price_inc_tax' => $this->num_f($line->unit_price, false, $business_details),
                'unit_price_exc_tax' => $this->num_f($line->unit_price, false, $business_details),

                //Fields for 4th column
                'line_total' => $this->num_f($line->height * $line->width * $line->unit_price * $line->quantity_returned, false, $business_details),
                'height' => $this->num_uf($line->height),
                'width' => $this->num_uf($line->width),
            ];

            $plate_line_return = [];
            foreach ($line->plate_line_return as $key => $value) {
                $plate_line_return[] = [
                    'width' => $value->width,
                    'height' => $value->height,
                    'quantity' => $value->quantity,
            ...
            $line_array['plate_line_return'] = $plate_line_return;
    ...
    public function getInvoiceNumber($business_id, $status, $location_id, $invoice_scheme_id = null)
    {
        if ($status == 'final') {
            if (empty($invoice_scheme_id)) {
                ...
                $scheme = InvoiceScheme::query()->lockForUpdate()->where('business_id', $business_id)
            ...
            $sell = Transaction::query()->where('invoice_no', $invoice_no)->first();
            if ($sell){
                \Log::info("Error: Duplicate invoice_no {$invoice_no}. Transaction: ". json_encode($sell));
                return null;
            }
    ...
    private function getInvoiceScheme($business_id, $location_id)
    {
        ...
        if (!empty($scheme_id) && $scheme_id != 0) {
            $scheme = InvoiceScheme::query()->lockForUpdate()->find($scheme_id);
        }

        //Check if scheme is not found then return default scheme
        if (empty($scheme)) {
            $scheme = InvoiceScheme::query()->where('business_id', $business_id)
                ->where('is_default', 1)
                ->lockForUpdate()
                ->first();
        }
    ...
    public function getPurchaseProducts($business_id, $transaction_id)
    {
        $products = Transaction::join('purchase_lines as pl', 'transactions.id', '=', 'pl.transaction_id')
            ...
            ->select('p.id as product_id', 'p.name as product_name', 'v.id as variation_id', 'v.name as variation_name', 'pl.quantity_line as quantity')
            ->get();
        return $products;
    }
    ...
    public function getSellTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        ...
        $output['total_sell_tax'] = $sell_details->sum('tax_amount');
...
//                ->select(DB::raw('SUM(IF( is_return = 0, amount, amount*-1))as total_paid'))
            ->select(DB::raw('SUM(IF( type <> "expense", amount, amount*-1))as total_paid'))
            ->first()
            ->total_paid;
     ...
     * @return int
     */
    public function getTotalPaidV1($transaction_id)
    {
        $total_paid = TransactionPayment::where('transaction_id', $transaction_id)
//                ->select(DB::raw('SUM(IF( is_return = 0, amount, amount*-1))as total_paid'))
        ...
        return $total_paid->sum('total_paid');
    }
     ...
     */
    public function mapNewPurchaseSell($business, $plate_lines, $mapping_type = 'purchase', $check_expiry = true, $purchase_line_id = null)
    {
        if (empty($plate_lines)) {
            return false;
        }
        ...
        foreach ($plate_lines as $plate_line) {
            $line = PlateStock::query()->find($plate_line->selected_plate_stock_id);
            $sellLine = TransactionSellLine::query()->find($plate_line->transaction_sell_line_id);
            ...
            $qty_sum_query = $this->get_pl_quantity_sum_string('PL');
            $qty_selling = $plate_line->width * $plate_line->height * $plate_line->quantity;
            ...
            $query = Transaction::join('purchase_lines AS PL', 'transactions.id', '=', 'PL.transaction_id')
                ...
                ->where('PL.product_id', $line->product_id)
                ->where('PL.variation_id', $line->variation_id)
                ->whereRaw('PL.height = ' . $line->height)
                ->whereRaw('PL.width = ' . $line->width)
                ->where('PL.warehouse_id', $line->warehouse_id);
...
//DB::connection()->enableQueryLog();
            ...
            $purchase_sell_map = [];
//var_dump(json_encode(DB::connection()->getQueryLog()));die;
            //Iterate over the rows, assign the purchase line to sell lines.
            foreach ($rows as $k => $row) {
                //Check if qty_available is more or equal
                if ($qty_selling <= $row->quantity_available) {
                ...
                if ($mapping_type == 'stock_adjustment') {
                    ...
                    //Mapping of purchase
                    $purchase_sell_map[] = ['plate_line_id' => $plate_line->id,
                        'purchase_line_id' => $row->purchase_lines_id,
                        'quantity' => $qty_allocated,
                        'created_at' => \Carbon::now(),
                        'updated_at' => \Carbon::now()
    ...
    public function canNotUpdate($transaction){
        $business_id = request()->session()->get('user.business_id');
        $business = Business::find($business_id);

        //Check if the transaction can be edited or not
        $can_not_edit_transaction = false;
        if ($transaction->status != 'draft' && !$this->canBeEdited($transaction->id, $business->transaction_edit_days)) {
            $can_not_edit_transaction = true;
        }

        //Check if the payment can be create or not
        $can_not_create_payment = false;
        if ($transaction->status != 'draft' && !$this->canBeEdited($transaction->id, $business->payment_create_days)) {
            $can_not_create_payment = true;
        }

        //Check if the payment can be approval or not
        $can_not_approval_payment = false;
        if ($transaction->status != 'draft' && !$this->canBeEdited($transaction->id, $business->payment_approval_days)) {
            $can_not_approval_payment = true;
        }
        ...
        return [
            'can_not_edit_transaction' => $can_not_edit_transaction,
            'can_not_create_payment' => $can_not_create_payment,
            'can_not_approval_payment' => $can_not_approval_payment,
        ];
    }
...
//            $multiplier = !empty($sub_unit['multiplier']) ? $sub_unit['multiplier'] : 1;
    ...
    public function getTransactionTotals(
        ...
        if (in_array('expense', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(IF(transactions.status='final' AND (SELECT transaction_expenses.type FROM transaction_expenses WHERE transaction_expenses.transaction_id = transactions.id LIMIT 1)='return_customer' AND (SELECT transaction_payments.approval_status FROM transaction_payments WHERE transaction_payments.transaction_id = transactions.id LIMIT 1)='approved', final_total, 0)) as expense_for_customer"),
                DB::raw("SUM(IF(transactions.status='final' AND (SELECT transaction_expenses.type FROM transaction_expenses WHERE transaction_expenses.transaction_id = transactions.id LIMIT 1)='expense' AND (SELECT transaction_payments.approval_status FROM transaction_payments WHERE transaction_payments.transaction_id = transactions.id LIMIT 1)='approved', final_total, 0)) as other_expense")
            );
        }
        ...
        if (in_array('stock_adjustment', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(IF(transactions.type='stock_adjustment' AND transactions.status='received', final_total, 0)) as total_adjustment"),
                DB::raw("SUM(IF(transactions.type='stock_adjustment' AND transactions.status='received', total_amount_recovered, 0)) as total_recovered")
            );
        }
        ...
        if (in_array('sell', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(IF(transactions.type='sell' AND transactions.status='final', discount_amount, 0)) as total_sell_discount"),
        ...
        if (in_array('expense', $transaction_types)) {
            $output['expense_for_customer'] =
                !empty($transaction_totals->expense_for_customer) ?
                    $transaction_totals->expense_for_customer : 0;

            $output['other_expense'] =
                !empty($transaction_totals->other_expense) ?
                    $transaction_totals->other_expense : 0;
        }
    ...
    public function getListSells($business_id)
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ->leftJoin('transaction_sell_lines as tsl', 'transactions.id', '=', 'tsl.transaction_id')
            ->leftJoin('users as u', 'transactions.created_by', '=', 'u.id')
            ->leftJoin('transaction_ships', 'transaction_ships.transaction_id', '=', 'transactions.id')
            ->leftJoin('users as ss', 'transaction_ships.ship_id', '=', 'ss.id')
//                ->leftJoin('res_tables as tables', 'transactions.res_table_id', '=', 'tables.id')
            ...
            ->where('transactions.business_id', $business_id)
            ->where('transactions.type', 'sell')
            ->whereIn('transactions.status', ['final', 'cancel'])
            ->select([
                'transactions.plates_sort_order',
                'transactions.status',
                'transactions.is_deliver',
                'transactions.export_status',
                DB::raw("GROUP_CONCAT(ss.first_name,' ',ss.last_name) AS shipper"),
                ...
                'contacts.mobile',
                'transactions.payment_status',
                DB::raw("IF(transactions.status <> 'cancel', transactions.final_total, 0) as final_total"),
                'transactions.flag_deliver',
                ...
                'transactions.is_approved_by_cashier',
                'transactions.is_confirm_debit_paper',
                DB::raw('DATE_FORMAT(transactions.transaction_date, "%Y/%m/%d") as sale_date'),
                DB::raw("CONCAT(COALESCE(u.surname, ''),' ',COALESCE(u.first_name, ''),' ',COALESCE(u.last_name,'')) as added_by"),
                DB::raw('(SELECT SUM(IF(TP.is_return = 1,-1*TP.amount,TP.amount)) FROM transaction_payments AS TP WHERE
                        TP.transaction_id=transactions.id AND TP.approval_status = "approved" AND TP.approval_status <> "reject") as total_paid'),

                DB::raw('(SELECT SUM(IF(TP.is_return = 1,-1*TP.amount,TP.amount)) FROM transaction_payments AS TP WHERE
                        TP.transaction_id=transactions.id AND TP.approval_status <> "reject") as total_paid_inc_unapprove'),

                DB::raw('(SELECT IF(TP.is_return = 1,-1*TP.amount,TP.amount) FROM transaction_payments AS TP WHERE
                        TP.transaction_id=transactions.id AND TP.type = "deposit" AND TP.approval_status <> "reject" ORDER BY TP.id LIMIT 1) as deposit'),
                DB::raw('(SELECT TP.approval_status FROM transaction_payments AS TP WHERE
                        TP.transaction_id=transactions.id AND TP.type = "deposit" AND TP.approval_status <> "reject" LIMIT 1) as deposit_approved'),
                DB::raw('(SELECT TP.id FROM transaction_payments AS TP WHERE
                        TP.transaction_id=transactions.id AND TP.type = "deposit" AND TP.approval_status <> "reject" LIMIT 1) as deposit_id'),

                DB::raw('(SELECT IF(TP.is_return = 1,-1*TP.amount,TP.amount) FROM transaction_payments AS TP WHERE
                        TP.transaction_id=transactions.id AND TP.type = "cod" AND TP.approval_status <> "reject" ORDER BY TP.id LIMIT 1) as cod'),
                DB::raw('(SELECT TP.approval_status FROM transaction_payments AS TP WHERE
                        TP.transaction_id=transactions.id AND TP.type = "cod" AND TP.approval_status <> "reject" LIMIT 1) as cod_approved'),
                DB::raw('(SELECT TP.id FROM transaction_payments AS TP WHERE
                        TP.transaction_id=transactions.id AND TP.type = "cod" AND TP.approval_status <> "reject" LIMIT 1) as cod_id'),

                DB::raw('(SELECT IF((SELECT SUM(IF(TP.is_return = 1,-1*TP.amount,TP.amount)) FROM transaction_payments AS TP WHERE
                        TP.transaction_id=transactions.id) >= transactions.final_total AND transactions.status <> "cancel", IF(COUNT(1) > 0, "pending", "approved"), IF(COUNT(1) > 0, "pending", "unapproved")) FROM transaction_payments AS TP WHERE
                        TP.transaction_id=transactions.id AND TP.type <> "cod" AND TP.type <> "deposit" AND TP.approval_status = "pending" LIMIT 1) as total_remaining_approved'),

                DB::raw('(SELECT IF(COUNT(TP.id) > 0, "pending", "approved") FROM transaction_payments AS TP WHERE
                        TP.transaction_id=transactions.id AND approval_status = "pending" LIMIT 1) as payment_approved'),

                'bl.name as business_location',
                DB::raw("SUM(IF(SR.status NOT IN ('reject', 'cancel'), 1, 0)) as return_exists"),
                DB::raw('(SELECT SUM(TP2.amount) FROM transaction_payments AS TP2 WHERE
    ...
    public function getLedgerDetails($contact_id, $start, $end)
    {
        ...
        $previous_transaction_sums = $this->__transactionQuery($contact_id, $start)
            ->select(
                ...
                DB::raw("SUM(IF(type = 'sell_return' AND status = 'final', final_total, 0)) as total_sell_return"),
        ...
        foreach ($transactions as $transaction) {
            $ledger[] = [
                ...
                'debit' => '',
                'credit' => '',
                'others' => $transaction->status == 'cancel' && $transaction->type == 'sell_return' ? 'Hủy đơn' : $transaction->additional_notes,
            ];
        }

        $invoice_sum = $transactions->where('type', 'sell')->where('status', '<>', 'cancel')->sum('final_total');
        $purchase_sum = $transactions->where('type', 'purchase')->sum('final_total');
        $sell_return_sum = $transactions->where('type', 'sell_return')->where('status', '<>', 'cancel')->sum('final_total');
        ...
        $prev_payments_sum = $this->__paymentQuery($contact_id, $start)
            ->select(
                DB::raw("SUM(IF(transaction_payments.approval_status = 'approved', transaction_payments.amount, 0)) as total_paid")
            )
            ->first();

        $payment_for = '';
        if ($transaction_types['payment'] != 'receipt') {
            $payment_for = __('lang_v1.payment') . ' ' . __('sale.receipt');
        } elseif ($transaction_types['payment'] != 'expense') {
            $payment_for = __('lang_v1.payment') . ' ' . __('sale.expense');
        }
        ...
        foreach ($payments as $payment) {
            ...
            $ledger[] = [
                ...
                'others' => !empty($ref_no) ? $payment->note . __('account.payment_for') . ': ' . $ref_no : $payment_for,
//                'status' => isset($payment->status) ? $payment->status : null
    ...
    public function calculateTotalMoneyOnDay($date) {
    ...
    public function getQueryOpeningStock($compareTime, $selectField){
        return "(SELECT $selectField FROM purchase_lines WHERE purchase_lines.id = PL.id $compareTime)";
    }

    public function getQueryImportStock($compareTime, $selectField){
        return "(SELECT $selectField FROM purchase_lines WHERE purchase_lines.id = PL.id $compareTime)";
    }

    public function getQueryExportStock($businessId, $filterLocation, $compareTime, $selectField){
        return "(SELECT $selectField FROM transaction_sell_lines_purchase_lines TSLPL 
                    LEFT JOIN purchase_lines PL1 ON PL1.id = TSLPL.purchase_line_id
                    WHERE $compareTime AND PL1.variation_id = v1.id AND TPL.transaction_sell_line_id = TSLPL.sell_line_id)";
    }

    public function getQueryEndStock($businessId, $filterLocation, $compareTime, $selectField){
        return "(SELECT $selectField FROM transactions LEFT JOIN transaction_sell_lines TSL ON TSL.transaction_id = transactions.id WHERE
                        transactions.type IN ('opening_stock', 'purchase') AND transactions.status = 'final' $compareTime $filterLocation
                        AND transactions.business_id = $businessId AND TSL.variation_id = variations.id)";
    }

    public function updatePlateStock($transaction)
    {
        foreach($transaction->stock_adjustment_lines as $stock_adjustment_line){
            ...
            //TODO: Update current plate stock
            $current_plate_stock->qty_available += $stock_adjustment_line->quantity_line;
            $current_plate_stock->save();

            if ($stock_adjustment_line->width != 0 && $stock_adjustment_line->height != 0 && in_array($product->unit->type, ['area', 'meter'])) {
                ...
                if (!empty($existPlateStock) && $existPlateStock->qty_available >= $stock_adjustment_line->quantity_line) {
    ...
    public function queryReportExportImport($compareStartTime, $comparePlateStartTime, $dateTimeQuery, $comparePlateEndTime, $plate_time_filter, $compareEndTime, $category_id){
        $fields = "export.product,
                    export.variation_id,
                    export.width,
                    export.height,
                    export.type,
                    export.unit,
                    export.sku,
                    export.export_begin_quantity,
                    export.begin_export_price,
                    export.export_quantity,
                    export.export_price,
                    export.end_export_quantity,
                    export.end_export_price,
                    purchase.product AS purchase_product,
                    purchase.width AS purchase_width,
                    purchase.height AS purchase_height,
                    purchase.type AS purchase_type,
                    purchase.unit,
                    purchase.unit_type,
                    purchase.sku AS purchase_sku,
                    purchase.begin_quantity,
                    purchase.begin_price,
                    purchase.import_quantity,
                    purchase.import_price,
                    purchase.end_stock_price";

        $queryImport = $this->queryImport($compareStartTime, $dateTimeQuery, $compareEndTime, $category_id);
        $queryExport = $this->queryExport($comparePlateStartTime, $plate_time_filter, $comparePlateEndTime, $category_id);

        return DB::select(DB::raw("
                        SELECT
                            $fields
                        FROM
                            ($queryImport) purchase
                        LEFT JOIN 
                            ($queryExport) export ON export.variation_id = purchase.variation_id AND export.height = purchase.height AND export.width = purchase.width
                        ...
                        RIGHT JOIN 
                            ($queryExport) export ON export.variation_id = purchase.variation_id AND export.height = purchase.height AND export.width = purchase.width
                        WHERE 
                            ((export.variation_id IS NOT NULL OR purchase.variation_id IS NOT NULL) AND (purchase.begin_price + purchase.import_quantity - export.export_quantity) >= 0)
        "));
    }

    public function queryImport($compareStartTime, $dateTimeQuery, $compareEndTime, $category_id){
        $condition = "";
        ...
        return "SELECT
                        SUM(IF($compareStartTime, pl.quantity, 0)) AS begin_quantity,
                        SUM(IF($compareStartTime, pl.quantity * pl.purchase_price_inc_tax, 0)) AS begin_price,
                        SUM(IF($dateTimeQuery, pl.quantity, 0)) AS import_quantity,
                        SUM(IF($dateTimeQuery, pl.quantity * pl.purchase_price_inc_tax, 0)) AS import_price,
                        SUM(IF($compareEndTime, pl.quantity * pl.purchase_price_inc_tax, 0)) AS end_stock_price,
                    ...
                    FROM
                        transactions
                        LEFT JOIN purchase_lines pl ON pl.transaction_id = transactions.id
                        LEFT JOIN variations v ON v.id = pl.variation_id
                        LEFT JOIN products p ON p.id = v.product_id 
                        LEFT JOIN units u ON p.unit_id = u.id 
                    WHERE
                        transactions.type IN ( 'purchase', 'opening_stock' ) 
                        AND transactions.`status` NOT IN ( 'cancel', 'draft' ) 
                        $condition
                    GROUP BY
                        pl.variation_id,
                        pl.width,
    ...
    public function queryExport($comparePlateStartTime, $plate_time_filter, $comparePlateEndTime, $category_id){
        $condition = "";
        if (!empty($category_id)) {
            $condition .= " AND p.category_id = " . $category_id;
        }
        ...
        return "SELECT
                        SUM(IF($comparePlateStartTime, tpl.total_quantity, 0)) AS export_begin_quantity,
                        SUM(IF($comparePlateStartTime, tpl.total_quantity * pl.purchase_price_inc_tax, 0)) AS begin_export_price,
                        SUM(IF($plate_time_filter, tpl.total_quantity, 0)) AS export_quantity,
                        SUM(IF($plate_time_filter, tpl.total_quantity * pl.purchase_price_inc_tax, 0)) AS export_price,
                        SUM(IF($comparePlateEndTime, tpl.total_quantity, 0)) AS end_export_quantity,
                        SUM(IF($comparePlateEndTime, tpl.total_quantity * pl.purchase_price_inc_tax, 0)) AS end_export_price,
                        IF( p.type = 'variable', CONCAT( p.NAME, '-', v.NAME ), p.`name` ) AS product,
                        v.id AS variation_id,
                        transactions.id AS transaction_id,
                        tpl.width,
                    ...
                    FROM
                        transactions
                        LEFT JOIN transaction_plate_lines tpl ON tpl.transaction_id = transactions.id
                        LEFT JOIN transaction_sell_lines_purchase_lines tslpl ON tslpl.plate_line_id = tpl.id
                        LEFT JOIN purchase_lines pl ON pl.id = tslpl.purchase_line_id
                        LEFT JOIN variations v ON v.id = tpl.variation_id
                        LEFT JOIN products p ON p.id = v.product_id 
                        LEFT JOIN units u ON p.unit_id = u.id 
                    WHERE
                        transactions.type IN ( 'sell' ) 
                        AND transactions.`status` NOT IN ( 'cancel', 'draft' )
                        $condition 
                    GROUP BY
                        tpl.variation_id,
    ...
    public function getQueryStockDeliver($id, $transaction, $location_id, $business_id) {
        ...
        join(
            ...
            ->select(
                ...
                DB::raw('IF(short_name, CONCAT(actual_name, " (", short_name, ")"), actual_name) as unit_name'),
...
//                $sell_details[$key]->sub_units_options = $this->productUtil->getSubUnits($business_id, $value->unit_id, false, $value->product_id);
                ...
                $sell_details[$key]->formatted_qty_available = $this->num_f($value->qty_available);

                $lot_numbers = [];
                if (request()->session()->get('business.enable_lot_number') == 1) {
                    $lot_number_obj = $this->getLotNumbersFromVariation($value->variation_id, $business_id, $location_id);
                ...
                $plate_stocks = PlateStock::leftjoin('products as p', 'plate_stocks.product_id', '=', 'p.id')
                    ...
                    ->where('variations.id', $sell_details[$key]->variation_id)
                    ->where('plate_stocks.width', '>=', floatval($sell_details[$key]->width))
                    ->where('plate_stocks.height', '>=', floatval($sell_details[$key]->height))
                    ->whereRaw('
                        (SELECT
                            SUM(qty_available) * ' . floatval($sell_details[$key]->width) . ' * ' . floatval($sell_details[$key]->height) . '
                        FROM
                            plate_stocks
                        WHERE
                            variation_id = ' . $sell_details[$key]->variation_id . ' AND
                            width >= ' . floatval($sell_details[$key]->width) . ' AND
                            height >= ' . floatval($sell_details[$key]->height) . ')
                        >= ' . floatval($sell_details[$key]->quantity * $sell_details[$key]->width * $sell_details[$key]->height) . '
                    ')
                    ->groupBy('variations.id')
                    ->groupBy('plate_stocks.width')
                    ->groupBy('plate_stocks.height')
                    ->get();
                $plate_stocks->toArray();

                $permitted_warehouses = auth()->user()->getPermittedWarehouses();
                if ($permitted_warehouses != 'all') {
                    $sell_details[$key]->warehouses = Warehouse::forDropdown($business_id, false, [], $permitted_warehouses);
                }else{
                    $sell_details[$key]->warehouses = Warehouse::forDropdown($business_id);
                }
//                $sell_details[$key]->warehouses = Warehouse::forDropdown($business_id);

                if($sell_details[$key]->warehouses){
                    $sell_details[$key]->warehouse_id = array_key_first($sell_details[$key]->warehouses->toArray());
                }
            }
        }
    ...
    public function getSellFinalTotal($transaction)
    {
        $sell_total = 0;
        foreach ($transaction->sell_lines as $sell_line){
            $quantity = (in_array($sell_line->sub_unit->type, ['meter', 'area']) && $sell_line->sub_unit->is_default == 1) ? $sell_line->width * $sell_line->height * $sell_line->quantity_line : $sell_line->quantity_line;
            $line_total = $quantity * $sell_line->unit_price;
            $line_total = round_int($line_total, 4);
            $sell_total += $line_total;
        }
        $sell_total = round($sell_total / 1000) * 1000;

        $final_total = $sell_total - $transaction->discount_amount + $transaction->tax_amount + $transaction->shipping_charges;
        $final_total = round($final_total / 1000) * 1000;

        return $final_total;
    }

    public function getUserConfirmPayment($cashier_confirmed_id, $admin_confirmed_id)
    {
        if($cashier_confirmed_id && $admin_confirmed_id){
            $type = 'both';
        }elseif($cashier_confirmed_id){
            $type = 'cashier';
        }else{
            $type = 'none';
        }

        if($cashier_confirmed_id){
            $cashier = User::withTrashed()->find($cashier_confirmed_id);
            if (empty($cashier)) {
                $cashier_name = '';
            } else {
                $cashier_name = implode(' ', [$cashier->first_name, $cashier->last_name]);
                $cashier_name = trim($cashier_name).' - '. $cashier->username;
            }
        }else{
            $cashier_name = '';
        }

        if($admin_confirmed_id){
            $admin = User::withTrashed()->find($admin_confirmed_id);
            if (empty($admin)) {
                $admin_name = '';
            } else {
                $admin_name = implode(' ', [$admin->first_name, $admin->last_name]);
                $admin_name = trim($admin_name).' - '. $admin->username;
            }
        }else{
            $admin_name = '';
        }
    ...
    public function queryTransfer($business_id){
        return Transaction::query()->leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ->leftJoin('transaction_ships', 'transaction_ships.transaction_id', '=', 'transactions.id')
            ->leftJoin('users as ss', 'transaction_ships.ship_id', '=', 'ss.id')
            ->where('transactions.business_id', $business_id)
            ->where('transactions.type', 'sell')
