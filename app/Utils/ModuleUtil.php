...
class ModuleUtil extends Util
{
        ...
        if (!empty($installed_modules)) {
            foreach ($installed_modules as $module) {
                ...
                if (class_exists($class)) {
                    ...
                    if (method_exists($class_object, $function_name) && !empty($arguments)) {
    ...
    public function accountsDropdown($business_id, $prepend_none = false, $closed = false, $show_balance = false, $type = '')
    {
        $dropdown = [];

        if ($this->isModuleEnabled('account')) {
            $dropdown = Account::forDropdown($business_id, $prepend_none, $closed, $show_balance, $type);
        }
...
//                    ]
    ...
    public function isClosedEndOfDay()
    {
        $business_id = request()->session()->get('user.business_id');
        $business = Business::where('id', $business_id)
            ->where('is_active', 1)
            ->whereNotNull('last_closed_at')
            ->whereRaw("DATE(last_closed_at) = CURDATE()")
            ->first();
    ...
    public function getUserClosedEndOfDay($date = null)
    {
        $result = null;
        $business_id = request()->session()->get('user.business_id');
        $business = Business::where('id', $business_id)
            ->where('is_active', 1)
            ->whereNotNull('last_closed_at')
            ->whereRaw("DATE(last_closed_at) = ?", [$date])
            ->first();

        if($business){
            $user = User::where('id', $business->last_closed_by)
                ->select(
                    DB::raw("CONCAT(first_name, ' ', last_name) as full_name")
                )
                ->first();
            $result = [
                'id' => $business->last_closed_by,
                'full_name' => $user->full_name,
                'closed_at' => $business->last_closed_at,
            ];
        }
    ...
    /*public function getUserClosedEndOfDay($date = '')
    {
        $business_id = request()->session()->get('user.business_id');
        $business = Business::where('id', $business_id)
            ->where('is_active', 1)
        ...
        $business = $business->whereRaw("DATE(last_closed_at) = CURDATE()")
            ->first();

        if($business){
            $user = User::find($business->last_closed_by);
        }else{
            $user = null;
        }
