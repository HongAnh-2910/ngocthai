...
class ContactUtil
{
    ...
    public function getContactInfo($business_id, $contact_id)
    {
        $contact = Contact::where('contacts.id', $contact_id)
            ...
            ->select(
                ...
                DB::raw("SUM(IF((t.type = 'sell') AND t.status = 'final', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id AND approval_status='approved'), 0)) as invoice_sale_received"),
                ...
                DB::raw("SUM(IF(t.type = 'sell_return' AND t.status != 'cancel', (SELECT SUM(amount) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id AND transaction_payments.approval_status = 'approved'), 0)) as invoice_return"),
                DB::raw("SUM(IF(t.type = 'opening_balance', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as opening_balance_paid"),
                DB::raw("SUM(IF((t.type = 'receipt') AND t.status = 'final', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id AND approval_status='approved'), 0)) as receipt_amount"),
                DB::raw("SUM(IF(t.type = 'purchase', final_total, 0)) as total_purchase"),
    ...
    public function getAllContactInfo($business_id)
    {
...
//            ->join('transaction_payments AS tp', 't.id', '=', 'tp.transaction_id')
            ...
            ->select(
                ...
                'contacts.*', 't.id AS transaction_id', 't.type AS transaction_type'
            )
            ->groupBy('contacts.id')
            ->get();

//        foreach ($contacts as $contact) {
//            $debtTransaction = TransactionPayment::where('transaction_id', );
//        }

        return $contacts;
    }

    public function getQueryDebt($business_id, $contact_id) {
        $contact = Contact::where('contacts.id', $contact_id)
            ->where('contacts.business_id', $business_id)
            ->join('transactions AS t', 'contacts.id', '=', 't.contact_id')
            ->with(['business'])
