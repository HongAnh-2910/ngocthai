...
class ProductUtil extends Util
{
     ...
     * @return boolean
     */
    public function createSingleProductVariation($product, $sku, $purchase_price, $dpp_inc_tax, $profit_percent, $selling_price, $selling_price_inc_tax, $combo_variations = [], $default_sell_price_by_plate = null)
    {
        ...
        $variation_data = [
                ...
                'default_sell_price_by_plate' => $default_sell_price_by_plate ? $this->num_uf($default_sell_price_by_plate) : null,
            ];
        ...
        foreach ($input_variations as $key => $value) {
            ...
            if (!empty($value['variations'])) {
                ...
                foreach ($value['variations'] as $k => $v) {
                    ...
                    if (!empty($v['price_groups'])) {
                        $price_groups[] = $v['price_groups'];
                    }
                }
                ...
                foreach ($variations as $variation) {
                    ...
                    if (!empty($price_groups[$i])) {
                        $this->createOrUpdateSellingPrices($variation, $price_groups[$i]);
                    }
                    $i++;
                }
            }
        ...
        foreach ($input_variations_edit as $key => $value) {
            ...
            if (!empty($value['variations_edit'])) {
                foreach ($value['variations_edit'] as $k => $v) {
                    ...
                    $this->createOrUpdateSellingPrices($variation, $v['price_groups']);
            ...
            if (!empty($value['variations'])) {
                ...
                foreach ($new_variations as $new_variation) {
                    ...
                    $this->createOrUpdateSellingPrices($new_variation, $price_groups[$i]);
                    $i++;
                }
            }
        }
    ...
    public function getDetailsFromVariation($variation_id, $business_id, $location_id = null, $check_qty = true)
    {
        $query = Variation::join('products AS p', 'variations.product_id', '=', 'p.id')
                ...
                ->leftjoin('units as su', 'p.default_sub_unit_id', '=', 'su.id')
        ...
        $product = $query->select(
            ...
            'variations.default_sell_price_by_plate',
            ...
            'units.width as unit_width',
            'units.height as unit_height',
            DB::raw('IF(su.width IS NULL, 1, su.width) as sub_unit_width'),
            DB::raw('IF(su.height IS NULL, 1, su.height) as sub_unit_height'),
//            'su.width as sub_unit_width',
//            'su.height as sub_unit_height',
    ...
    public function calculateInvoiceTotal($products, $tax_id, $discount = null)
    {
        ...
        foreach ($products as $product) {
            $is_default_unit = Unit::find($product['sub_unit_id'])->is_default;
            $area = $this->num_uf($product['width']) *  $this->num_uf($product['height']) *  $this->num_uf($product['quantity']);

            if(isset($product['line_discount_amount'])){
                //Calculate discount by product
                $output['discount'] += $this->num_uf($product['line_discount_amount']) * $area;
                if ($is_default_unit == 0) {
                    $output['total_before_tax'] += round_int(($this->num_uf($product['default_sell_price']) - $this->num_uf($product['line_discount_amount'])) * $this->num_uf($product['quantity']), 4);
                } else {
                    $output['total_before_tax'] += round_int(($this->num_uf($product['default_sell_price']) - $this->num_uf($product['line_discount_amount'])) * $area, 4);
                }
            }else{
                if ($is_default_unit == 0) {
                    $output['total_before_tax'] += round_int($this->num_uf($product['default_sell_price']) * $this->num_uf($product['quantity']), 4);
                } else {
                    $output['total_before_tax'] += round_int($this->num_uf($product['unit_price_inc_tax']) *  $area, 4);
                }
            }
        ...
        $output['total_before_tax'] = round($output['total_before_tax'] / 1000) * 1000;;

        //Calculate discount by total order
...
//                        ->value('price_inc_tax');
                        ->select(['price_inc_tax', 'price_by_plate'])
                        ->first();

        $price_exc_tax = $price_inc_tax->price_inc_tax;
        if (!empty($price_inc_tax->price_inc_tax) && !empty($tax_id)) {
            $tax_amount = TaxRate::where('id', $tax_id)->value('amount');
            $price_exc_tax = $this->calc_percentage_base($price_inc_tax->price_inc_tax, $tax_amount);
        }

        return [
            'price_inc_tax' => $price_inc_tax->price_inc_tax,
            'price_by_plate' => $price_inc_tax->price_by_plate,
     ...
     * @return mixed
     */
    public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null, $action = 'store')
    {
        ...
        foreach ($input_data as $data) {
            if(!isset($data['is_origin'])){
                $data['is_origin'] = 0;
            }
            $multiplier = 1;
            $plate_line = $this->num_uf($data['quantity']);

            if (isset($data['sub_unit_id']) && $data['sub_unit_id'] == $data['product_unit_id']) {
                $data['quantity'] = $this->num_uf($data['width']) * $this->num_uf($data['height']) * $this->num_uf($data['quantity']);
            ...
            $old_qty = $oldWidth = $oldWarehouseId = $oldVariationId = $oldIsOrigin = 0;
            if (isset($data['purchase_line_id'])) {
                $purchase_line = PurchaseLine::findOrFail($data['purchase_line_id']);
                $purchase_line->purchase_price = $this->num_uf($data['purchase_price_inc_tax']);
                $purchase_line->purchase_price_inc_tax = $this->num_uf($data['purchase_price_inc_tax']);
                $updated_purchase_line_ids[] = $purchase_line->id;
                $old_qty = $this->num_uf($purchase_line->quantity_line);
                $oldWidth = $purchase_line->width;
                $oldWarehouseId = $purchase_line->warehouse_id;
                $oldVariationId = $purchase_line->variation_id;
                $oldIsOrigin = $purchase_line->is_origin;
            } else {
            ...
            $purchase_line->is_origin = $data['is_origin'];
            $purchase_line->warehouse_id = !empty($data['warehouse_id']) ? $data['warehouse_id'] : null;
            $purchase_line->width = !empty($data['width']) ? $data['width'] : null;
            $purchase_line->height = !empty($data['height']) ? $data['height'] : null;
            $purchase_line->quantity_line = $plate_line;
            ...
            $valid = $this->updatePlateStockForPurchaseLine($transaction, $purchase_line, $data, $plate_line, $old_qty, $oldWidth, $oldWarehouseId, $oldVariationId, $oldIsOrigin, $action);
            if (!$valid) {
                return false;
            }
        }
            ...
            if ($delete_purchase_lines->count()) {
                foreach ($delete_purchase_lines as $delete_purchase_line) {
                    ...
                    if ($before_status == 'received') {
                        PlateStock::query()->where('location_id', $transaction->location_id)
                            ->where('variation_id', $delete_purchase_line->variation_id)
                            ->whereRaw('width = ' . $delete_purchase_line->width)
                            ->whereRaw('height = ' . $delete_purchase_line->height)
                            ->where('warehouse_id', $delete_purchase_line->warehouse_id)
                            ->where('is_origin', $delete_purchase_line->is_origin)
                            ->decrement('qty_available', $delete_purchase_line->quantity_line);
                    }
                }
    ...
    public function changePurchaseLineUnit($purchase_line, $business_id)
    {
        ...
        /*if (!empty($sub_unit)) {
            ...
            $purchase_line->purchase_price = $purchase_line->purchase_price_inc_tax * $multiplier;
...
//            $purchase_line->quantity = $purchase_line->quantity / ($purchase_line->width * $purchase_line->height);
        };*/
    ...
    public function createOrUpdateSellingPrices($variation, $input_price_groups){
        $variation_group_prices = [];
        foreach ($input_price_groups as $key => $value) {
            ...
            $variation_group_price->price_inc_tax = !empty($value) ? $this->num_uf($value) : 0;
    ...
    public function createOrUpdateSellingPricesByPlate($variation, $input_price_groups){
        $variation_group_prices = [];
        foreach ($input_price_groups as $key => $value) {
            $variation_group_price =
                VariationGroupPrice::where('variation_id', $variation->id)
            ...
            $variation_group_price->price_by_plate = !empty($value) ? $this->num_uf($value) : 0;
            $variation_group_prices[] = $variation_group_price;
        }
    ...
    public function updatePlateStockForPurchaseLine($transaction, $purchase_line, $data, $plate_line, $old_qty, $oldWidth, $oldWarehouseId, $oldVariationId, $oldIsOrigin, $action = ''){
        //Save to plate_stocks table
//        $is_origin = $oldIsOrigin ? $oldIsOrigin : $data['is_origin'];
        $oldQty = empty($old_qty) ? 0 : $old_qty;

        $plate_stock = PlateStock::where('location_id', $transaction->location_id)
            ->where('variation_id', $data['variation_id'])
            ->whereRaw('width = ' . $data['width'])
            ->whereRaw('height = ' . $purchase_line->height)
            ->where('warehouse_id', $data['warehouse_id'])
            ->where('is_origin', $data['is_origin'])
            ->first();

        if ($action == 'update') {
            $existTransaction = $this->checkExistTransaction($transaction, $purchase_line);
        ...
        if (isset($data['purchase_line_id']) && ($oldWidth != $data['width'] || $oldWarehouseId != $data['warehouse_id'] || $plate_line != $old_qty || $oldIsOrigin != $data['is_origin'])) {
            PlateStock::query()->where('location_id', $transaction->location_id)
                ->where('variation_id', $oldVariationId)
                ->whereRaw('width = ' . $oldWidth)
                ->whereRaw('height = ' . $purchase_line->height)
                ->where('warehouse_id', $oldWarehouseId)
                ->where('is_origin', $oldIsOrigin)
                ->decrement('qty_available', $old_qty);
        }

        if ($plate_stock) {
            //If exist plate
            if (empty($oldQty) || empty($plate_stock->qty_available) || (!empty($oldWidth) && $oldWidth != $data['width'])
                || (!empty($oldWarehouseId) && $oldWarehouseId != $data['warehouse_id']) || $oldIsOrigin != $data['is_origin']) {
                $newQuantity = $plate_stock->qty_available + $plate_line;
            } else {
                $newQuantity = $plate_stock->qty_available - $oldQty + $plate_line;
            }

            if (isset($data['purchase_line_id']) && $newQuantity < 0) {
                return false;
            }

            $plate_stock->update([
                'qty_available' => $newQuantity,
//                'is_origin' => $purchase_line->is_origin,
            ]);
        } else {
            //If not exist plate
            PlateStock::create([
                ...
                'width' => $purchase_line->width,
                'height' => $purchase_line->height,
                'warehouse_id' => $purchase_line->warehouse_id,
                'qty_available' => $purchase_line->quantity_line,
                'is_origin' => $purchase_line->is_origin,
            ]);
        }
    ...
    public function decreasePlateStockForPurchaseLine($transaction, $purchase_line){
        $plate_stock = PlateStock::where('location_id', $transaction->location_id)
            ->where('variation_id', $purchase_line->variation_id)
            ->whereRaw('width = ' . $purchase_line->width)
            ->whereRaw('height = ' . $purchase_line->height)
            ->where('warehouse_id', $purchase_line->warehouse_id)
            ->where('is_origin', $purchase_line->is_origin)
            ->first();

        if ($plate_stock) {
            $newQuantity = $plate_stock->qty_available - $purchase_line->quantity_line;
            if ($newQuantity < 0) {
                return false;
            }

            $plate_stock->update(['qty_available' => $newQuantity]);
        }

        return true;
    }

    public function checkExistTransaction($transaction, $purchaseLine){
        $purchaseLines = PurchaseLine::query()->where('transaction_id', $transaction->id)->get();
        foreach ($purchaseLines as $purchase_line) {
            $condition = 'AND variation_id = ' . $purchase_line->variation_id . ' AND location_id = ' . $transaction->location_id . ' AND width = ' . $purchase_line->width . ' AND height = ' . $purchase_line->height;
            if (!empty($purchase_line->created_at)) {
                $checkCondition = Transaction::query()
                    ->whereIn('type', ['sell', 'stock_adjustment', 'purchase_transfer', 'sell_transfer'])
                    ->where('transactions.created_at', '>=', date('Y-m-d H:i:s', strtotime($purchase_line->created_at)))
                    ->whereRaw('(EXISTS (SELECT 1 FROM purchase_lines pl WHERE pl.transaction_id = transactions.id ' . $condition  . ' AND warehouse_id = ' . $purchase_line->warehouse_id . ') 
                OR EXISTS (SELECT 1 FROM transaction_sell_lines tsl WHERE tsl.transaction_id = transactions.id ' . $condition . ') 
                OR EXISTS (SELECT 1 FROM stock_adjustment_lines sal WHERE sal.transaction_id = transactions.id AND variation_id = ' . $purchase_line->variation_id . '))'
                    )
                    ->get()
                    ->toArray();
            }

            if (!empty($checkCondition)) {
    ...
    public function addOpeningStock($opening_stock, $product, $business_id)
    {
        ...
        DB::table('purchase_lines')->insert([
            ...
            'quantity' => $opening_stock['quantity'] * $opening_stock['height'] * $opening_stock['width'],
            'width' => $opening_stock['width'],
            'height' => $opening_stock['height'],
            'quantity_line' => $opening_stock['quantity'],
            'is_origin' => $opening_stock['is_origin'],
            'pp_without_discount' => 0,
            'item_tax' => 0,
            'tax_id' => null,
            'purchase_price' => 0,
            'purchase_price_inc_tax' => 0 + $item_tax,
            'exp_date' => null,
        ...
        // Create plate stock table
        $plate_stock = PlateStock::where('location_id', $opening_stock['location_id'])
            ->where('variation_id', $product->variation_id)
            ->where('width', $opening_stock['width'])
            ->where('height', $opening_stock['height'])
            ->where('warehouse_id', $opening_stock['warehouse_id'])
            ->where('is_origin', $opening_stock['is_origin'])
            ->first();

        if ($plate_stock) {
            //If exist plate
            $plate_stock->update([
                'qty_available' => $plate_stock->qty_available + $opening_stock['quantity'],
            ]);
        } else {
            //If not exist plate
            PlateStock::create([
