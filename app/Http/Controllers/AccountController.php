            ...
            $accounts = Account::leftjoin('account_transactions as AT', function ($join) {
                ...
                ->leftJoin('users AS u', 'accounts.created_by', '=', 'u.id')
                                ...
                                ->select(['accounts.name', 'accounts.account_number', 'accounts.note', 'accounts.id', 'accounts.account_type_id',
                                    ...
                                    'is_closed',
                                    DB::raw("SUM(IF(tp.approval_status = 'approved', tp.amount, 0)) as balance"),
    ...
    public function update(Request $request, $id)
    {
        ...
        if (request()->ajax()) {
            try {
                ...
                $account->account_number = isset($input['account_number']) ? $input['account_number'] : '';
...
//                    DB::raw("(SELECT SUM(IF(TP.type='expense', TP.amount, -1 * TP.amount)) from account_transactions as AT JOIN accounts as ac ON ac.id=AT.account_id WHERE ac.business_id= $business_id AND AT.operation_date <= account_transactions.operation_date AND AT.deleted_at IS NULL) as balance"),
                    DB::raw("(SELECT SUM(AT.amount) from account_transactions as AT JOIN accounts as ac ON ac.id=AT.account_id WHERE ac.business_id= $business_id AND AT.operation_date <= account_transactions.operation_date AND AT.deleted_at IS NULL) as balance"),
                    'account_transactions.transaction_id',
                    'account_transactions.id',
    ...
    public function __getPaymentDetails($row)
    {
        ...
        if (!empty($row->sub_type)) {
            ...
            if (!empty($row->transaction->type)) {
                ...
                }elseif ($row->transaction->type == 'receipt') {
                    $details = __('sale.receipt') . '<br><b>' . __('purchase.ref_no') . ':</b> ' . $row->transaction->ref_no;
                } elseif ($row->transaction->type == 'sell') {
                    $details = __('sale.sale') . '<br><b>' . __('contact.customer') . ':</b> <a href="' . action("ContactController@show", [$row->transaction->contact->id]) . '" target="_blank">' . $row->transaction->contact->name . '</a><br><b>'.
