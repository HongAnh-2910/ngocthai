...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $warehouses = Warehouse::with('plate_stocks.variation.product.unit')->get();

        $plate_stocks = PlateStock::leftjoin('variations', 'plate_stocks.variation_id', '=', 'variations.id')
            ...
            ->select(
                ...
                DB::raw('SUM(plate_stocks.height * plate_stocks.width * plate_stocks.qty_available) as total_area'),
            ...
            ->havingRaw('total_area < alert_quantity')
            ->get();

        if (!empty($warehouses)) {
            foreach ($warehouses as $warehouse) {
                $plates = [];
                foreach ($plate_stocks as $key => $plate_stock) {
                    if ($warehouse->id == $plate_stock->warehouse_id) {
                        $plates[] = [
                            'product' => $plate_stock->product,
                            'total_area_remaining' => $plate_stock->total_area,
                            'unit_type' => $plate_stock->unit_type,
                            'unit_name' => $plate_stock->unit_name
                        ];
                    }
                }

                $warehouse->plates = $plates;
            }
        }

        return view('home.index', compact('date_filters', 'sells_chart_1', 'sells_chart_2', 'widgets', 'all_locations', 'warehouses'));
    }
...
//            $output['total_expense'] = $transaction_totals['total_expense'];
    ...
    public function getProductStockAlert(Request $request)
    {
        ...
        /*if ($request->ajax()) {
            ...
            $query = PlateStock::leftjoin('products as p', 'plate_stocks.product_id', '=', 'p.id')
                ...
                ->whereRaw('(plate_stocks.width * plate_stocks.height * plate_stocks.qty_available) <= p.alert_quantity');

            $products = $query->select(
                'units.type as unit_type',
                DB::raw('IF(units.short_name, CONCAT(units.actual_name, " (", units.short_name, ")"), units.actual_name) as unit_name'),
            ...
            $datatable =  Datatables::of($products)
                ...
                ->editColumn('stock', function ($row) {
                    if($row->unit_type == 'area'){
                        $html = number_format($row->stock * $row->width * $row->height, 2) . ' m<sup>2</sup>';
                    }else{
    ...
    public function getSalesPaymentDues()
    {
        if (request()->ajax()) {
            ...
            return Datatables::of($dues)
                ->addColumn('due', function ($row) {
                    $total_paid = !empty($row->total_paid) ? $row->total_paid : 0;
                    $due = round_int($row->final_total - $total_paid, env('DIGIT', 4));
                    return '<span class="display_currency" data-currency_symbol="true">' .
                    $due . '</span>';
                })
    ...
    public function loadMoreNotifications()
    {
        ...
        foreach ($notifications as $notification) {
            ...
            if (in_array($notification->type, [\App\Notifications\RecurringInvoiceNotification::class, \App\Notifications\TransactionPaymentNotification::class])) {
                $msg = '';
                $icon_class = '';
                $link = '';
                ...
                }elseif($notification->type == \App\Notifications\TransactionPaymentNotification::class){
                    $notification_data = $this->notificationUtil->getTransactionPayment($notification);
                    $msg = $notification_data['msg'];
                    $icon_class = $notification_data['icon_class'];
                    $link = action('SellPosController@showTransactionPayment', [$notification->id]);
                }
