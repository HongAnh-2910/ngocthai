            ...
            return Datatables::of($purchases_returns)
                ->addColumn('action', function ($row) {
                    ...
                    if (!empty($row->return_parent_id)) {
                        $html .= '<a href="' . action('PurchaseReturnController@add', $row->return_parent_id) . '" class="btn btn-info btn-xs mr-2" ><i class="glyphicon glyphicon-edit ml-1"></i>' .
                                __("messages.edit") .
                                '</a>';
                    } else {
                        $html .= '<a href="' . action('CombinedPurchaseReturnController@edit', $row->id) . '" class="btn btn-info btn-xs" ><i class="glyphicon glyphicon-edit ml-1"></i>' .
                                __("messages.edit") .
                                '</a>';
                    }

                    $html .= '<a href="' . action('PurchaseReturnController@destroy', $row->id) . '" class="btn btn-danger btn-xs delete_purchase_return" ><i class="fa fa-trash ml-1"></i>' .
                                __("messages.delete") .
                                '</a>';
    ...
    public function destroy($id)
    {
        ...
        try {
            if (request()->ajax()) {
                ...
                $output = ['success' => true,
                            'msg' => __('lang_v1.purchase_return_deleted_success')
                        ];
            }
        } catch (\Exception $e) {
            DB::rollBack();
