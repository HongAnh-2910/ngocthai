    ...
    */
    ...
    public function __construct(BusinessUtil $businessUtil, RestaurantUtil $restaurantUtil, ModuleUtil $moduleUtil)
    {
        ...
        $this->theme_colors = [
            ...
            'red' => 'Đỏ',
            'yellow' => 'Vàng',
            'blue-light' => 'Xanh dương nhạt',
            'black-light' => 'Đen nhạt',
            'purple-light' => 'Tím nhạt',
            'green-light' => 'Xanh lá nhạt',
            'red-light' => 'Đỏ nhạt',
        ];
...
//            );
            ...
            $business_details['enabled_modules'] = ['purchases','add_sale','pos_sale','stock_transfers','stock_adjustment','expenses', 'service_staff', 'account'];
            dd($business_details);
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            $business_details = $request->only(['name', 'start_date', 'currency_id', 'tax_label_1', 'tax_number_1', 'tax_label_2', 'tax_number_2', 'default_profit_percent', 'default_sales_tax', 'default_sales_discount', 'sell_price_tax', 'sku_prefix', 'time_zone', 'fy_start_month', 'accounting_method', 'sales_cmsn_agnt', 'item_addition_method', 'currency_symbol_placement', 'on_product_expiry',
                'transaction_edit_days', 'payment_create_days', 'payment_approval_days',
            ...
            $business_details['max_weight_dont_need_confirm'] = !empty($request->input('max_weight_dont_need_confirm')) ? $request->input('max_weight_dont_need_confirm') : 0;
            $business_details['max_pcs_dont_need_confirm'] = !empty($request->input('max_pcs_dont_need_confirm')) ? $request->input('max_pcs_dont_need_confirm') : 0;
...
//            $business_details['custom_labels'] = json_encode($business_details['custom_labels']);
    ...
    public function closeEndOfDay(Request $request)
    {
        if (!auth()->user()->can('sell.close_end_of_day')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        try {
            ...
            $payment = TransactionPayment::where('business_id', $business_id)
                ...
                ->whereDate('paid_on', $current_date)
                ->first();
            if($payment){
                $output = [
                    'success' => 0,
                    'msg' => __('sale.need_confirm_transfer_method')
                ];
            ...
            if($sell_return){
                $output = [
                    'success' => 0,
                    'msg' => __('sale.need_confirm_sell_return')
                ];
            ...
            //Check approved stock adjustment
            $sell_return = Transaction::where('business_id', $business_id)
            ...
            if($sell_return){
                $output = [
                    'success' => 0,
                    'msg' => __('sale.need_confirm_stock_adjustment')
                ];
            ...
            $business->last_closed_at = date('Y-m-d H:i:s');
            $business->last_closed_by = auth()->user()->id;
            $business->save();

            $output = [
                'success' => 1,
                'msg' => __('messages.close_end_of_day_success')
            ];
        } catch (\Exception $e) {
            \Log::emergency("File:" . $e->getFile(). "Line:" . $e->getLine(). "Message:" . $e->getMessage());
            $output = [
