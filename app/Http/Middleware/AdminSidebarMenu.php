...
//                        }
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('sell.add_reduce_debt')) {
                            $sub->url(
                                action('ReduceDebtController@index'),
                                __('contact.confirm_reduce_debt'),
                                ['icon' => 'fa fas fa-arrow-down', 'active' => request()->segment(1) == 'contacts' && request()->segment(2) == 'reduce-debts']
                            );
                        }
            ...
            if (in_array('purchases', $enabled_modules) && (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create') || auth()->user()->can('purchase.update'))) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        if (auth()->user()->can('purchase.view') || auth()->user()->can('view_own_purchase') || auth()->user()->can('sell.create_return_bill') || auth()->user()->can('sell.receipt_expense')) {
                            $sub->url(
                        ...
                        if (auth()->user()->can('purchase.create') || auth()->user()->can('sell.create_return_bill') || auth()->user()->can('sell.receipt_expense')) {
                            $sub->url(
            ...
            //Sell dropdown
            if (auth()->user()->can('sell.view')  || auth()->user()->can('stock.view_deliver_orders') || auth()->user()->can('sell.create') || auth()->user()->can('direct_sell.access') || auth()->user()->can('view_own_sell_only') || auth()->user()->can('sell.receipt_expense')) {
                $menu->dropdown(
                    __('sale.sale'),
                    function ($sub) use ($enabled_modules) {
                        ...
                        if (in_array('add_sale', $enabled_modules) && auth()->user()->can('sell.create')) {
                            $sub->url(
                        ...
                        if (auth()->user()->can('sell.receipt_expense')) {
                            $sub->url(
                                action('SellController@sellsOfCashier'),
                                __('lang_v1.Cashier'),
                                ['icon' => 'fas fa-chalkboard-teacher fa', 'active' => request()->segment(1) == 'sells-of-cashier' && request()->segment(2) == null]
                            );
                        }
                        ...
                        if (auth()->user()->can('stock.view_deliver_orders')) {
                            $sub->url(
                                ...
                                __('lang_v1.storekeeper'),
                                ['icon' => 'fa fas fa-file-export', 'active' => request()->segment(1) == 'stock-to-deliver' && request()->segment(2) == null]
                            );
                        }
                        ...
                        if (auth()->user()->can('return.list') || auth()->user()->can('sell.view')) {
                            $sub->url(
                        ...
                        if (auth()->user()->can('sell.receipt_expense')) {
                            $sub->url(
                                action('SellPosController@listTransactionPayments'),
                                __('sale.payment_confirm'),
                                ['icon' => 'fa fas fa-dollar-sign', 'active' => request()->segment(1) == 'sells-of-cashier' && request()->segment(2) == 'transaction-payments']
                            );
                        }
...
//                        }
            ...
            if (in_array('stock_transfers', $enabled_modules) && (auth()->user()->can('stock.create_stock_transfer_bill'))) {
                $menu->dropdown(
                    __('lang_v1.stock_transfers'),
                    function ($sub) {
                        if (auth()->user()->can('stock.create_stock_transfer_bill')) {
                            $sub->url(
            ...
            if (in_array('stock_adjustment', $enabled_modules) && (auth()->user()->can('stock.create_stock_adjustment_bill'))) {
                $menu->dropdown(
                    __('stock_adjustment.stock_adjustment'),
                    function ($sub) {
                        if (auth()->user()->can('stock.create_stock_adjustment_bill')) {
                            $sub->url(
...
//            }
            ...
            if(auth()->user()->can('target.view') || auth()->user()->can('target.create') ) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        $sub->url(
                            action('TargetController@index'),
                            __('target.list_target'),
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'targets' && request()->segment(2) == null]
                        );
                        if (auth()->user()->can('target.create')) {
                            $sub->url(
                                action('TargetController@create'),
                                __('target.add_target'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'targets' && request()->segment(2) == 'create']
                            );
                        }
                    },
                    ['icon' => 'fa fa-bullseye']
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules) {
                        if (auth()->user()->can('report.revenue_date')) {
                            $sub->url(
                                action('ReportController@receiptRevenueReport'),
                                __('report.revenue_report'),
                                ['icon' => 'fa fas fa-calendar-alt', 'active' => request()->segment(2) == 'revenue-by-date-report']
                            );
                            $sub->url(
                                action('ReportController@revenueByMonthReport'),
                                __('report.revenue_by_month_report'),
                                ['icon' => 'fa fas fa-calendar-alt', 'active' => request()->segment(2) == 'revenue-by-month-report']
                            );
                        }
                        if (auth()->user()->can('report.transfer')) {
                            $sub->url(
                                action('ReportController@transferReport'),
                                __('report.transfer_report'),
                                ['icon' => 'fa fas fa-shipping-fast', 'active' => request()->segment(2) == 'transfer-report']
                            );
                        }
                        /*if (auth()->user()->can('report.reporting_date')) {
                            $sub->url(
                                action('ReportController@reportingDate'),
                                __('report.reporting_date'),
                                ['icon' => 'fa fas fa-calendar-alt', 'active' => request()->segment(2) == 'reporting-date']
                            );
                        }*/
...
//                            $sub->url(
//                                action('ReportController@reportExportImport'),
//                                __('report.import_export_report'),
//                                ['icon' => 'fa fas fa-hourglass-half', 'active' => request()->segment(2) == 'report-import-export']
//                            );
                            $sub->url(
                                action('ReportController@totalSales'),
                                __('report.report_total_sale'),
                                ['icon' => 'fa fas fa-hourglass-half', 'active' => request()->segment(2) == 'total-sales']
                            );
...
//                            );
//                        }
                        if (auth()->user()->can('target.report-owner-target')) {
                            $sub->url(
                                action('ReportController@reportOwnerTarget'),
                                __('report.report_owner_target'),
                                ['icon' => 'fa fas fa-chart-line', 'active' => request()->segment(2) == 'report-owner-target']
                            );
                        }
                        /*<li class="{{ $request->segment(2) == 'report-owner-target' ? 'active' : '' }}" ><a href="{{action('ReportController@reportOwnerTarget')}}"><i class="fa fas fa-chart-line" aria-hidden="true"></i>@lang('report.report_owner_target')</a></li>*/
                    },
                    ...
                    function ($sub) use ($enabled_modules) {
                        ...
                        if (auth()->user()->can('warehouse.view')) {
                            $sub->url(
                                ...
                                ['icon' => 'fa fas fa-warehouse', 'active' => request()->segment(1) == 'warehouses']
                            );
                        }
