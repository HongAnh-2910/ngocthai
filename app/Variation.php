...
class Variation extends Model
{
    ...
    /**
     * Returns list of product in array format
     * @param $productIds
     * @param $variationIds
     * @return Collection
     */
    public static function dropdownForAward($productIds = [], $variationIds = [])
    {
        $query = Product::leftJoin('variations', 'variations.product_id', '=', 'products.id')
            ->select([
                DB::raw("IF(variations.`name` = 'DUMMY', products.name, CONCAT(products.`name`, ' - ', variations.`name`)) AS pro_name"),
                'variations.id'
            ])
            ->pluck('pro_name', 'variations.id');
