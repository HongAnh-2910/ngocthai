...
class User extends Authenticatable
{
    ...
    public function permitted_warehouses()
    {
        $user = $this;
        ...
        if ($user->can('access_all_locations')) {
            ...
            $all_locations = BusinessLocation::with('warehouses')->where('business_id', $business_id)->get();
            foreach ($all_locations as $location) {
                if ($user->can('location.' . $location->id)) {
                    if ($user->can('location.' . $location->id . '.access_all_warehouses')) {
                        $permitted_warehouses[] = 'location.' . $location->id . '.access_all_warehouses';
                    } else {
    ...
    public function getPermittedWarehouses()
    {
        ...
        if ($user->can('access_all_locations')) {
            ...
            foreach ($all_locations as $location) {
                if ($user->can('location.' . $location->id)) {
                    foreach ($location->warehouses as $warehouse) {
                        if ($user->can('location.' . $location->id . '.access_all_warehouses') || $user->can('warehouse.' . $warehouse->id)) {
                            $permitted_warehouses[] = $warehouse->id;
                        }
                    }
                }
     ...
     * @return array users
     */
    public static function forDropdownSell($business_id, $prepend_none = true, $include_commission_agents = false, $prepend_all = false)
    {
        $query = User::permission('sell.create')->orWhereHas('roles', function ($q) {
            $q->where('name', 'LIKE', '%Admin%');
        })->where('business_id', $business_id)->user();
